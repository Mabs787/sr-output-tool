...
- `www-sky-com-deals`: Phase B corrected clear OCR/caption noise and added reusable engine support for single-select listboxes, compact result-count splitting, product-card image button grouping, visual separator list items, and trailing disclaimer button groups; remaining gaps are footer tail group announcements around country selector/back-to-top traversal.
- `www-sky-com-glass-43-inch`: Phase B corrected monthly price spacing, a truncated table column phrase, and FAQ casing; remaining page-backed gaps include strong-text benefit-list segmentation and native comparison-table traversal.
