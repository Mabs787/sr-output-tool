...
export function createDomScanner(options: DomScannerOptions): DomScanner {
  ...
  function visibleAssignedElementNode(node: any): boolean {
    return (
      node?.nodeType === Node.ELEMENT_NODE &&
      !isHidden(node) &&
      !(node.tagName?.toLowerCase() === "template" && node.getAttribute("shadowrootmode"))
    );
  }

  function isCustomLinkLikeHost(el: any): boolean {
    if (!visibleAssignedElementNode(el)) return false;
    if (!isCustomElement(el) || !hasShadowRootContent(el)) return false;
    if (implicitRole(el) === "link") return true;
    return hasSerializedShadowDescendantLink(el);
  }

  function axConfirmsAssignedTextBeforeCustomLink(
    host: any,
    text: string,
    linkHost: any,
  ): boolean {
    ...
    if (!hostAxNode || !linkHostDomId) return false;

    const flattened: AccessibilityTreeNode[] = [];
    const visit = (node?: AccessibilityTreeNode): void => {
      if (!node) return;
      flattened.push(node);
      for (const childId of node.childIds || []) {
    ...
    const textIndex = flattened.findIndex(
      (node) =>
        !node.ignored &&
        normalizedAxRole(node.role) === "statictext" &&
        normalize(node.name) === text,
    );
    const linkHostIndex = flattened.findIndex(
      (node) => normalize(node.domNodeId) === linkHostDomId,
    );

    return textIndex >= 0 && linkHostIndex > textIndex;
  }

  function assignedSlotDirectParagraphTextBeforeCustomLinkStops(
    slot: any,
  ): Array<{ node: any; text: string }> | undefined {
    if (slot?.tagName?.toLowerCase() !== "slot") return undefined;
    if (normalize(slot.getAttribute("name"))) return undefined;

    const host = shadowContentHostByNode.get(slot);
    if (!isCustomElement(host) || !hasShadowRootContent(host)) return undefined;

    const nodes = assignedSlotNodes(slot);
    const stops: Array<{ node: any; text: string }> = [];
    ...
    for (let index = 0; index < nodes.length; index += 1) {
      ...
      if (!text || !/[\p{L}\p{N}]/u.test(text) || !/[.!?]/u.test(text)) continue;

      const previousElement = nodes
        .slice(0, index)
        .reverse()
        .find((candidate: any) => visibleAssignedElementNode(candidate));
      const nextElement = nodes
        .slice(index + 1)
        .find((candidate: any) => visibleAssignedElementNode(candidate));

      if (!previousElement || implicitRole(previousElement) !== "heading") continue;
      if (!isCustomLinkLikeHost(nextElement)) continue;
      if (!axConfirmsAssignedTextBeforeCustomLink(host, text, nextElement)) continue;
  ...
  function labelElementForControlIncludingHidden(el: any): any | undefined {
    if (!el || el.nodeType !== Node.ELEMENT_NODE) return undefined;
    if ("labels" in el && el.labels?.length) {
      return Array.from(el.labels).find((label: any) =>
        Boolean(normalize(textWithoutInteractive(label) || readableText(label))),
      );
    }
  ...
  function labelTextForControlIncludingHidden(el: any): string | undefined {
    const label = labelElementForControlIncludingHidden(el);
    return label ? normalize(textWithoutInteractive(label) || readableText(label)) : undefined;
  }

  function describedByElementsForControl(el: any): any[] {
  ...
  function firstDescribedByTextForControl(el: any): string | undefined {
  ...
  function axFocusableControlNodeForControl(
    el: any,
    role: string,
    labelText: string,
    detailsText?: string,
  ): AccessibilityTreeNode | undefined {
    const expectedRole = role === "textbox" && el?.getAttribute?.("type") === "date"
      ? "date"
      : role;
    const exactNode = axNodeForElementRole(el, expectedRole);
    const matches = (node?: AccessibilityTreeNode): node is AccessibilityTreeNode => {
      if (!node || normalizedAxRole(node.role) !== expectedRole) return false;
      if (node.properties?.focusable !== true) return false;
      if (normalize(node.name) !== labelText) return false;
      if (detailsText && normalize(node.description) !== detailsText) return false;
      return true;
    };

    if (matches(exactNode)) return exactNode;
    return accessibilityNodes.find(matches);
  }

  function axHasStaticText(value?: string): boolean {
  ...
  function nativeFormControlsAcrossShadow(root: any): any[] {
    const controls: any[] = [];
  ...
  function customElementHasOnlyFormControlStructure(el: any): boolean {
  ...
  function isAnonymousCustomFormControlContainerGroup(el: any, role: string): boolean {
    ...
    if (!customElementHasOnlyFormControlStructure(el)) return false;
  ...
  function customFormControlHostSequence(el: any): Array<{
    source: string;
    el: any;
    announcement: string;
    role?: string;
    ...
    if (!isCustomElement(el)) return undefined;
    if (el.matches(interactiveSelector)) return undefined;
    if (
      el.getAttribute("role") ||
      el.hasAttribute("tabindex") ||
      normalize(el.getAttribute("aria-label")) ||
    ...
    const controls = nativeFormControlsAcrossShadow(el).filter((control) => {
      const tag = control.tagName?.toLowerCase();
      if (tag === "input" && (control.getAttribute("type") || "text").toLowerCase() === "hidden") {
        return false;
      }
      return ["combobox", "textbox", "searchbox"].includes(implicitRole(control));
    });
    ...
    if (!axNode || !axHasStaticText(labelText)) return undefined;
    if (hintText && !axHasStaticText(hintText)) return undefined;

    const descriptor = captureElement(control);
    const controlAnnouncement = descriptor ? generateAnnouncement(descriptor) : undefined;
    ...
    if (
      ...
      tag === "input" &&
      hintText &&
      normalize(control.getAttribute("aria-autocomplete")) === "list" &&
      (control.getAttribute("aria-haspopup") === "listbox" ||
        normalize(axNode.properties?.hasPopup as string) === "listbox")
    ) {
      return [
  ...
  function isLegendTextCoveredByFieldsetPrompt(el: any, role: string): boolean {
    if (role !== "text" && role !== "paragraph") return false;
    if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el)) return false;
    const legend = el.closest?.("legend");
    const fieldset = legend?.parentElement;
    if (!legend || fieldset?.tagName?.toLowerCase() !== "fieldset") return false;
    if (!visuallyHiddenFieldsetLegendText(fieldset)) return false;
  ...
  function urlBasenameLabel(
    ...
    const decoded = decodeURIComponent(
      options.preserveExtension ? lastSegment : lastSegment.replace(/\.[a-z0-9]+$/i, ""),
    ).replace(/_+/g, " ");
  ...
  function axConfirmedEmptyLinkUrlBasenameLabel(el: any, role: string): string | undefined {
    if (role !== "link" || el?.tagName?.toLowerCase() !== "a") return undefined;
    if (el.getAttribute("aria-label") || el.getAttribute("aria-labelledby")) return undefined;
    if (normalize(el.getAttribute("title")) || linkContentName(el)) return undefined;
  ...
  function isEmptyHeadingBeforeContentBoundary(el: any, role: string): boolean {
    ...
    if (firstMeaningfulDescendantIsNamedImage(next)) return true;
  ...
  function nextVisibleSiblingAcrossTransparentAncestors(el: any): any | undefined {
    for (let current = el, depth = 0; current && depth < 4; current = current.parentElement, depth += 1) {
      ...
      if (!isTransparentSingleMeaningfulChildWrapper(parent, current)) return undefined;
    }
    return undefined;
  }

  function isTransparentSingleMeaningfulChildWrapper(wrapper: any, child: any): boolean {
    if (!wrapper || wrapper.nodeType !== Node.ELEMENT_NODE || isHidden(wrapper)) return false;
    if (wrapper.matches?.(interactiveSelector)) return false;
    const role = implicitRole(wrapper);
    if (role && !["generic", "group"].includes(role)) return false;
    if (normalize(accessibleName(wrapper, role || "group") || directOwnText(wrapper))) return false;

    const meaningfulChildren = Array.from(wrapper.children || []).filter((candidate: any) => {
  ...
  function firstMeaningfulDescendantIsNamedImage(el: any): boolean {
    const first = firstVisibleDescendantMatching(el, (candidate: any) => {
      const role = implicitRole(candidate);
      if (role === "image" && accessibleName(candidate, "image")) return true;
      if (candidate.matches?.(interactiveSelector)) return true;
      if (role && !["generic", "group"].includes(role)) return true;
      return Boolean(
        normalize(directOwnText(candidate)) &&
          !candidate.querySelector?.("img, svg, video, canvas, [role='img']"),
      );
    });
  ...
  function firstMeaningfulArticleHeading(el: any): any | undefined {
    ...
    const visit = (node: any): void => {
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        if (normalize(node.textContent)) blocked = true;
        return;
      }

      if (node.nodeType !== Node.ELEMENT_NODE || isHidden(node)) return;
      if (node !== el && node.closest?.("article,[role='article']") !== el) return;
    ...
    if (
      !result ||
      result.closest?.("a[href],[role='link']") ||
      result.querySelector?.("a[href],[role='link']")
    ) {
      return undefined;
    }
    return result;
  ...
  function axEmptyArticleNameFromFirstMeaningfulHeading(el: any, role: string): string | undefined {
    if (role !== "article" || el?.tagName?.toLowerCase() !== "article") return undefined;
    if (
      el.getAttribute("aria-label") ||
      el.getAttribute("aria-labelledby") ||
      el.getAttribute("title")
    ...
    if (!axArticle || normalize(axArticle.name)) return undefined;
  ...
  function axEmptyArticleNameFromFirstEmptyOverlayLink(el: any, role: string): string | undefined {
    if (role !== "article" || el?.tagName?.toLowerCase() !== "article") return undefined;
    if (
      el.getAttribute("aria-label") ||
      el.getAttribute("aria-labelledby") ||
    ...
    if (!firstVisibleChild || implicitRole(firstVisibleChild) !== "link") return undefined;
  ...
  function directAxInlineTextLinkParagraphFragments(el: any): string[] | undefined {
    ...
    function shouldSkipInterLinkConjunction(index: number, text: string): boolean {
      if (text.toLowerCase() !== "or") return false;
      const firstLinkIndex = axChildren.findIndex(
        (node) => normalizedAxRole(node.role) === "link",
      );
      const leadingText = normalize(
        axChildren
      ...
      const nextLinkIndex = axChildren.findIndex(
        (node, childIndex) =>
          childIndex > index && normalizedAxRole(node.role) === "link",
      );
      if (nextLinkIndex < 0) return false;
      const trailingText = normalize(
        axChildren
          .slice(nextLinkIndex + 1)
          .filter((node) => normalizedAxRole(node.role) === "statictext")
          .map((node) => node.name || "")
          .join(" "),
      );
      if (trailingText && /[\p{L}\p{N}]/u.test(trailingText)) return false;
      return (
        normalizedAxRole(axChildren[index - 1]?.role) === "link" &&
        normalizedAxRole(axChildren[index + 1]?.role) === "link"
      );
    ...
    function shouldJoinInterLinkConjunction(index: number, text: string): boolean {
      if (text.toLowerCase() !== "or") return false;
      const firstLinkIndex = axChildren.findIndex(
        (node) => normalizedAxRole(node.role) === "link",
      );
    ...
    function pushAxChild(child: AccessibilityTreeNode, index: number): void {
      ...
      if (shouldSkipInterLinkConjunction(index, text)) return;
  ...
  function strongWrappedOnlyNativeLinkParagraphFragments(el: any): string[] | undefined {
    if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el)) return undefined;
    if (el.tagName.toLowerCase() !== "p") return undefined;
    if (
      el.getAttribute("role") ||
    ...
    const directElements = Array.from(el.children || []).filter((child: any) => !isHidden(child));
    if (directElements.length !== 1) return undefined;

    const wrapper = directElements[0] as any;
    if (!["strong", "b", "em", "i"].includes(wrapper.tagName?.toLowerCase())) {
      return undefined;
    }
    if (normalize(directOwnText(el)) || wrapper.querySelector?.("br")) return undefined;
    ...
    const disallowedInteractive = Array.from(wrapper.querySelectorAll(interactiveSelector)).some(
      (descendant: any) => descendant !== links[0],
    );
    if (disallowedInteractive) return undefined;

    let textOutsideLink = "";
    const collectTextOutsideLink = (node: any): void => {
      if (!node || node === links[0]) return;
      if (node.nodeType === Node.TEXT_NODE) {
        textOutsideLink += node.textContent || "";
        return;
      }
      if (node.nodeType !== Node.ELEMENT_NODE) return;
      if (node.matches?.("a[href], [role='link']")) return;
      for (const child of Array.from(node.childNodes || [])) {
    ...
    if (outsideText && /[\p{L}\p{N}]/u.test(outsideText)) return undefined;

    const paragraphAxNode = axNodeForElementRole(el, "paragraph");
    const paragraphAxChildren = axChildNodes(paragraphAxNode);
    if (!paragraphAxNode || paragraphAxChildren.length !== 1) return undefined;
    ...
    if (wrapperAxChildren.length < 1 || wrapperAxChildren.length > 2) return undefined;
    const [linkAxNode, trailingAxNode] = wrapperAxChildren;
    if (normalizedAxRole(linkAxNode.role) !== "link") return undefined;
    if (linkAxNode.properties?.focusable !== true) return undefined;
    ...
    if (trailingAxNode) {
      if (normalizedAxRole(trailingAxNode.role) !== "statictext") return undefined;
      const trailingText = normalize(trailingAxNode.name);
      if (trailingText && /[\p{L}\p{N}]/u.test(trailingText)) return undefined;
    }

    const linkName = normalizeAnnouncementLabel(linkAxNode.name);
    return linkName ? [`link, ${linkName}`] : undefined;
  }
  ...
  function firstShadowInclusiveDescendantMatching(
    el: any,
  ...
  function hasShadowInclusiveDescendantMatching(
    el: any,
    predicate: (candidate: any) => boolean,
  ): boolean {
  ...
  function isUnnamedElementForRole(el: any, role: string): boolean {
    if (accessibleName(el, role)) return false;
    return !normalize(
      el?.getAttribute?.("aria-label") ||
        textFromIdRefs(el?.getAttribute?.("aria-labelledby")) ||
  ...
  function hasExplicitOrAxName(el: any, role: string): boolean {
    if (
      normalize(
        el?.getAttribute?.("aria-label") ||
          textFromIdRefs(el?.getAttribute?.("aria-labelledby")) ||
  ...
  function hasNamedNavigationOrListDescendant(el: any): boolean {
    return hasShadowInclusiveDescendantMatching(el, isNamedNavigationOrList);
  }

  function hasVisibleLinkOrButtonDescendant(el: any): boolean {
  ...
  function hasVisibleFooterTextDescendant(el: any): boolean {
    return hasShadowInclusiveDescendantMatching(el, (candidate) => {
      const tag = candidate.tagName?.toLowerCase();
  ...
  function containsNativeLandmark(el: any, tagName: "header" | "footer"): boolean {
    if (el?.tagName?.toLowerCase() === tagName) return true;
    return hasShadowInclusiveDescendantMatching(
      el,
      (candidate) => candidate.tagName?.toLowerCase() === tagName,
    );
  }

  function hasMainAncestorAcrossShadow(el: any): boolean {
    return Boolean(shadowInclusiveAncestor(el, "main,[role='main']"));
  }

  function isUnsupportedUnnamedShadowHeaderFooterWrapper(el: any, role: string): boolean {
    if (!["group", "banner", "contentinfo", "sectionfooter"].includes(role)) return false;
    if (hasExplicitOrAxName(el, role)) return false;
    if (el?.matches?.(interactiveSelector)) return false;
    ...
    const isHeaderWrapper =
      role === "banner" ||
      tag === "header" ||
      (role === "group" && containsNativeLandmark(el, "header"));
    const isFooterWrapper =
      role === "contentinfo" ||
      role === "sectionfooter" ||
      tag === "footer" ||
      (role === "group" && containsNativeLandmark(el, "footer"));

    if (!isHeaderWrapper && !isFooterWrapper) return false;
    ...
    return (
      hasVisibleLinkOrButtonDescendant(el) &&
      (hasNamedNavigationOrListDescendant(el) ||
        hasShadowInclusiveDescendantMatching(
          el,
          (candidate) => ["ul", "ol", "dl"].includes(candidate.tagName?.toLowerCase()),
        ) ||
        hasVisibleFooterTextDescendant(el))
    );
  }

  function isUnsupportedUnnamedHeaderControlGroup(el: any, role: string): boolean {
    if (role !== "group") return false;
    if (!isUnnamedElementForRole(el, role)) return false;
    if (el?.matches?.(interactiveSelector)) return false;
    if (!shadowInclusiveAncestor(el, "header,[role='banner']")) return false;
    if (!hasVisibleLinkOrButtonDescendant(el)) return false;
    if (hasNamedNavigationOrListDescendant(el)) return false;
  ...
  function axSingleLinkChild(el: any): AccessibilityTreeNode | undefined {
    const axNode =
      axNodeForElementRole(el, "generic") ||
      axNodeAnyForElement(el);
    if (!axNode) return undefined;
    const axRole = normalizedAxRole(axNode.role);
    if (!axNode.ignored && axRole !== "generic") return undefined;
    if (normalize(axNode.name)) return undefined;
  ...
  function containsNativeFooterAcrossShadow(el: any): boolean {
    return containsNativeLandmark(el, "footer") || Boolean(el?.matches?.("footer,[role='contentinfo']"));
  }

  function closestAncestorContainingNativeFooter(el: any): any | undefined {
    const boundary = shadowInclusiveAncestor(el, "main,[role='main']") || document.body;
    const seen = new Set<any>();
    for (
      let current = el?.parentElement || shadowContentHostByNode.get(el);
      current && current !== boundary && !seen.has(current);
      current = current.parentElement || shadowContentHostByNode.get(current)
    ) {
      seen.add(current);
      if (containsNativeFooterAcrossShadow(current)) return current;
    }
    return undefined;
  }

  function nextVisibleElementAfterSubtree(el: any): any | undefined {
    const boundary = shadowInclusiveAncestor(el, "main,[role='main']") || document.body;
  ...
  function previousVisibleElementBeforeSubtree(el: any): any | undefined {
    const boundary = shadowInclusiveAncestor(el, "main,[role='main']") || document.body;
    for (let current = el; current && current !== boundary; current = current.parentElement || shadowContentHostByNode.get(current)) {
  ...
  function isFooterAdjacentSingleLinkGenericGroup(el: any): boolean {
    if (!axSingleLinkChild(el)) return false;
    const next = nextVisibleElementAfterSubtree(el);
    return Boolean(next && containsNativeFooterAcrossShadow(next));
  }

  function isFooterSingleLinkGenericGroup(el: any): boolean {
    if (!axSingleLinkChild(el)) return false;
    return Boolean(
      shadowInclusiveAncestor(el, "footer,[role='contentinfo']") ||
        closestAncestorContainingNativeFooter(el),
    );
  }

  function hasCardLikeContentShape(el: any): boolean {
    const hasHeading = hasShadowInclusiveDescendantMatching(
      el,
      (candidate) => implicitRole(candidate) === "heading",
    );
    if (!hasHeading) return false;

    const hasImage = hasNamedImageDescendant(el);
    const hasText = hasShadowInclusiveDescendantMatching(
      el,
      (candidate) =>
        ["paragraph", "text"].includes(implicitRole(candidate)) &&
        Boolean(readableText(candidate)),
    );
    const hasLink = hasShadowInclusiveDescendantMatching(
      el,
      (candidate) => implicitRole(candidate) === "link",
    );

    return (hasImage || hasText) && hasLink;
  }

  function hasNamedImageDescendant(el: any): boolean {
    return hasShadowInclusiveDescendantMatching(
      el,
      (candidate) => implicitRole(candidate) === "image" && Boolean(accessibleName(candidate, "image")),
    );
  }

  function isImmediatelyAfterVisibleLink(el: any): boolean {
    const previous = previousVisibleElementBeforeSubtree(el);
    if (!previous) return false;
    if (implicitRole(previous) === "link") return true;
    return hasShadowInclusiveDescendantMatching(
      previous,
      (candidate) => implicitRole(candidate) === "link",
    );
  ...
  function isUnsupportedLeadingGenericContentWrapper(el: any): boolean {
    if (!hasCardLikeContentShape(el)) return false;
    return !isImmediatelyAfterVisibleLink(el) || hasNamedImageDescendant(el);
  }

  function isUnsupportedUnnamedGenericContentWrapper(el: any, role: string): boolean {
    if (role !== "group") return false;
    if (!isUnnamedElementForRole(el, role)) return false;
    if (el?.matches?.(interactiveSelector)) return false;
    if (
      isFocusableImageListItem(el) ||
      isFocusableStructuredListItemGroup(el) ||
    ...
    const axRole = normalizedAxRole(axNode?.role);
    if (axNode && !axNode.ignored && axRole !== "generic") return false;
    if (axNode && normalize(axNode.name)) return false;

    return (
      isFooterAdjacentSingleLinkGenericGroup(el) ||
      isFooterSingleLinkGenericGroup(el) ||
      isUnsupportedLeadingGenericContentWrapper(el)
    );
  }
  ...
  function captureElement(el: any): CapturedElementDescriptor | null {
    ...
    const semanticArticleHeadingName =
      articleNameFromFirstHeading(el, role) || axEmptyArticleHeadingName;
    const articleHeadingName =
      axEmptyArticleOverlayLinkName ||
      semanticArticleHeadingName;
      ...
      inlineTextLinkFragments:
        role === "paragraph"
          ? footerInlineBoundaryParagraphFragments(el) ||
            ...
            strongWrappedOnlyNativeLinkParagraphFragments(el) ||
  ...
  function scanSubtree(root: any): ScanLogEntry[] {
    ...
    function walk(el: any): void {
      ...
      const assignedSlotTextStops =
        assignedSlotDirectParagraphTextBeforeCustomLinkStops(el);
      if (assignedSlotTextStops?.length) {
        const id = `__sr_el_${stopIndex}_${now()}`;
        stopIndex += 1;
        el.setAttribute("data-sr-id", id);
        const textStopsByNode = new Map(
          assignedSlotTextStops.map((stop) => [stop.node, stop.text] as const),
        );
        for (const child of assignedSlotNodes(el)) {
          ...
          if (text) {
            emitTraversalStop(
              id,
              syntheticTextStop("assigned-slot-direct-text-before-custom-link", el, text),
            );
            continue;
          }
