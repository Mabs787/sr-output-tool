...
export function createDomScanner(options: DomScannerOptions): DomScanner {
  ...
  function axRenderedDescendantTextCaseName(el: any, name?: string): string | undefined {
    ...
    if (!root.ignored && !["figcaption", "paragraph", "time"].includes(rootRole || "")) return undefined;
  ...
  function hasAxNodeForElementRole(el: any, role: string): boolean {
  ...
  function hasDecorativeMediaOnlyElement(el: any): boolean {
    if (!el || el.nodeType !== Node.ELEMENT_NODE) return false;
    const tag = el.tagName?.toLowerCase();
    if (tag === "svg") return el.getAttribute("aria-hidden") === "true" || !nestedImageLabel(el);
  ...
  function isPlainNativeButton(el: any, role = implicitRole(el)): boolean {
    if (role !== "button") return false;
    if (el?.tagName?.toLowerCase() !== "button") return false;
    if (el.disabled || el.hasAttribute?.("disabled") || el.getAttribute("aria-disabled") === "true") {
      return false;
  ...
  function nativeButtonSingleTextAndDecorativeMedia(el: any, role = implicitRole(el)): string | undefined {
    ...
    const visibleChildren = Array.from(el.children || []).filter((child: any) => !isHidden(child));
    const textChildren = visibleChildren.filter((child: any) => {
      if (child.matches?.(interactiveSelector)) return false;
      if (hasDecorativeMediaOnlyElement(child)) return false;
      return Boolean(normalize(readableText(child)));
    });
    if (textChildren.length !== 1) return undefined;
    if (
      !visibleChildren.every((child: any) =>
        textChildren.includes(child) || hasDecorativeMediaOnlyElement(child),
      )
    ) {
      return undefined;
    }
    ...
    return label && label === buttonName ? label : undefined;
  }

  function hasOnlyDecorativeMediaDescendants(el: any): boolean {
    if (!el || el.nodeType !== Node.ELEMENT_NODE) return false;
    const children = Array.from(el.children || []);
    if (!children.length) return false;
    return children.every((child: any) => {
      if (hasDecorativeMediaOnlyElement(child)) return true;
      if (child.matches?.(interactiveSelector)) return false;
  ...
  function hasPreviousCodeExampleBlock(el: any): boolean {
    for (let current = el?.parentElement, depth = 0; current && depth < 5; current = current.parentElement, depth += 1) {
      const pres = Array.from(current.querySelectorAll?.("pre") || []).filter(
        (pre: any) =>
          !isHidden(pre) &&
          Boolean(directPreCodeChild(pre)) &&
          Boolean(hasSyntaxHighlightedCodeDescendants(directPreCodeChild(pre))) &&
          Boolean(
            pre.compareDocumentPosition(el) &
              pre.ownerDocument.defaultView.Node.DOCUMENT_POSITION_FOLLOWING,
          ),
      );
  ...
  function isCodeExampleActionGroupButton(el: any, role = implicitRole(el)): boolean {
    if (!isPlainNativeButton(el, role)) return false;
    if (!hasPreviousCodeExampleBlock(el)) return false;
  ...
  function isFilterRowGroupButton(el: any, role = implicitRole(el)): boolean {
    if (!nativeButtonSingleTextAndDecorativeMedia(el, role)) return false;
    if (isFooterLegalActionButton(el, role)) return false;

    const wrapper = el.parentElement;
    const row = wrapper?.parentElement;
    if (!wrapper || !row || wrapper === document.body || row === document.body) return false;
    if (wrapper.matches?.(interactiveSelector) || row.matches?.(interactiveSelector)) return false;

    const rowButtons = Array.from(row.querySelectorAll(":scope > * > button")).filter(
      (button: any) => !isHidden(button) && Boolean(nativeButtonSingleTextAndDecorativeMedia(button)),
    );
    if (rowButtons.length < 3 || !rowButtons.includes(el)) return false;

    const labelCandidates = [
      ...Array.from(row.children || []),
      ...Array.from(row.parentElement?.children || []),
    ];
    return labelCandidates.some((child: any) => {
      if (isHidden(child) || child === wrapper || child === row) return false;
      if (!["p", "span", "div"].includes(child.tagName?.toLowerCase())) return false;
  ...
  function implicitRole(el: any): string {
    ...
    if (tag === "label" && isAxConfirmedStandaloneLabelTextGroup(el)) return "group";
    ...
    if (
      tag === "code" &&
      !el.closest("pre, p, li, h1, h2, h3, h4, h5, h6, [role='heading'], [role='button'], button, a[href]") &&
      !el.querySelector("*") &&
      normalize(readableText(el) || el.textContent)
    ) {
      return "text";
    }
    if (
    ...
    if (isAxConfirmedEmptyNamedGenericGroup(el)) return "group";
  ...
  function parentListPosition(el: any) {
    ...
    if (
      nativeMarkerListItemAnnouncements(parentItem) ||
      axOrderedParentNestedListItemAnnouncements(parentItem) ||
      axOrderedDirectTextListItemShouldDescend(parentItem) ||
      axOrderedBlockChildListItemMarkerAnnouncement(parentItem)
    ) {
      return {};
    }
  ...
  function positionInSet(el: any, role: string): number | undefined {
    ...
    if (
      ...
      !hasAxExposedAnonymousSvgListItemContent(el.closest("li,[role='listitem']"), "listitem") &&
    ...
    if (
      role === "paragraph" &&
      isDirectAxOrderedBlockChildListItemParagraph(el, role)
    ) {
      return undefined;
    }
  ...
  function axRawChildNodes(node?: AccessibilityTreeNode): AccessibilityTreeNode[] {
    return (node?.childIds || [])
      .map((childId) => accessibilityNodeById.get(normalize(childId) || ""))
      .filter((child): child is AccessibilityTreeNode => Boolean(child));
  }

  function nativeAxListMarkerRawText(markerNode?: AccessibilityTreeNode): string | undefined {
  ...
  function nativeAxListMarkerText(markerNode?: AccessibilityTreeNode): string | undefined {
  ...
  function nativeAxNestedListMarkerText(markerNode?: AccessibilityTreeNode): string | undefined {
    const marker = nativeAxListMarkerRawText(markerNode);
    if (!marker) return undefined;
    if (/^\d+[.)]$/u.test(marker)) return marker;
    return marker === "◦" ? "." : "•";
  ...
  function nativeAxMarkerAnnouncement(
  ...
  function isAxBackedNativeOrderedListItem(el: any): boolean {
    ...
    if (el.getAttribute("role")) return false;
    if (el.getAttribute("aria-label") || el.getAttribute("aria-labelledby")) return false;
    if (el.getAttribute("data-sr-marker-content") !== "normal") return false;
    const markerDisplay = normalize(el.getAttribute("data-sr-marker-display"));
    if (markerDisplay !== "inline" && markerDisplay !== "inline-block") return false;
    if (normalize(el.getAttribute("data-sr-marker-list-style-type")) !== "decimal") {
      return false;
    }
    if (el.matches?.("[aria-live], [aria-disabled='true'], [hidden]")) return false;
    ...
    if (list.getAttribute("role") && list.getAttribute("role") !== "list") return false;
    if (list.closest("nav,[role='navigation']")) return false;
    if (isMarkerSeparatedLinkList(list)) return false;
  ...
  function axOrderedBlockChildListItemMarkerAnnouncement(el: any): string | undefined {
    if (!isAxBackedNativeOrderedListItem(el)) return undefined;
    if (normalize(directOwnText(el))) return undefined;

    const directChildren = Array.from(el.children || []).filter((child: any) => !isHidden(child));
    if (!directChildren.length) return undefined;
    if (
      directChildren.some((child: any) => {
        const tag = child.tagName?.toLowerCase();
    ...
    if (!directChildren.some((child: any) => child.tagName?.toLowerCase() === "p")) {
      return undefined;
    }
    ...
    return nativeAxMarkerAnnouncement(
      markerNode,
      positionInSet(el, "listitem"),
      setSize(el, "listitem"),
    );
  }

  function isDirectAxOrderedBlockChildListItemParagraph(
    el: any,
    role = implicitRole(el),
  ): boolean {
    if (role !== "paragraph" || el.tagName?.toLowerCase() !== "p") return false;
  ...
  function axOrderedDirectTextListItemAnnouncements(el: any): string[] | undefined {
    if (!isAxBackedNativeOrderedListItem(el)) return undefined;
    if (axPlainTextMarkerListItemAnnouncement(el)) return undefined;
    if (axOrderedParentNestedListItemAnnouncements(el)) return undefined;
    if (el.querySelector("p")) return undefined;
    ...
    if (directLinks.length && directNestedLists.length) return undefined;
    if (
      directSemanticChildren(el).some(
        (child: any) => !directLinks.includes(child) && !directNestedLists.includes(child),
      )
    ) {
      return undefined;
    }
    ...
    const marker = nativeAxListMarkerText(markerNode);
    if (!marker || !/^\d+[.)]$/u.test(marker)) return undefined;
    if (normalizedAxRole(firstNode.role) !== "statictext") return undefined;

    const leadingText = normalize(firstNode.name);
    if (!leadingText || leadingText !== directText) return undefined;

    const position = positionInSet(el, "listitem");
    const size = setSize(el, "listitem");
    const nestedListSuffix =
      directNestedLists.length && position && size ? `, ${position} of ${size}` : "";
    ...
    for (const node of followingNodes) {
      ...
      if (role === "list") {
        ...
        if (
          !nestedList ||
          sawNestedList ||
          normalize(node.domNodeId) !==
            normalize(nestedList.getAttribute("data-sr-dom-node-id"))
        ) {
          return undefined;
        }
        sawNestedList = true;
      ...
      if (sawNestedList) return undefined;
      if (role === "link") {
        ...
        if (
          ...
          normalize(node.domNodeId) !== normalize(link.getAttribute("data-sr-dom-node-id"))
        ) {
          return undefined;
        }
    ...
    if (linkIndex !== directLinks.length) return undefined;
    if (nestedListIndex !== directNestedLists.length) return undefined;
    return fragments;
  }

  function directOrderedTextListItemNestedLists(el: any): any[] {
    return Array.from(el.children || []).filter(
      (child: any) => !isHidden(child) && implicitRole(child) === "list",
    );
  }

  function axOrderedDirectTextListItemShouldDescend(el: any): boolean {
    return Boolean(
      axOrderedDirectTextListItemAnnouncements(el)?.length &&
        directOrderedTextListItemNestedLists(el).length,
    );
  }

  function axNativeInlineChildBoundaryListItemAnnouncements(el: any): string[] | undefined {
    if (!isListItem(el) || el.tagName?.toLowerCase() !== "li") return undefined;
    if (el.getAttribute("role")) return undefined;
    if (el.getAttribute("aria-label") || el.getAttribute("aria-labelledby")) return undefined;
    if (el.matches?.(interactiveSelector) || el.hasAttribute?.("tabindex") || el.tabIndex >= 0) {
    ...
    for (const child of Array.from(el.childNodes || [])) {
      ...
      const tag = child.tagName?.toLowerCase();
      if (tag === "br") continue;
      if (directLinks.includes(child) || metadataElements.includes(child)) continue;
    ...
    if (!hasEmphasisBoundary) return undefined;

    const axListItem = axNodeForElementRole(el, "listitem");
    if (!axListItem || normalize(axListItem.name)) return undefined;
    if (axListItem.properties?.focusable === true) return undefined;

    const axChildren = axRawChildNodes(axListItem);
    if (axChildren.length < 5) return undefined;

    const [markerNode, ...contentNodes] = axChildren;
    const markerAnnouncement = nativeAxMarkerAnnouncement(
      markerNode,
      positionInSet(el, "listitem"),
      setSize(el, "listitem"),
    );
    ...
    function pushText(value?: string): void {
      ...
      if (metadataText) {
        const separator = needsBoundary(metadataText, text)
          ? " "
          : /^[,.;:!?)]/u.test(text)
            ? ""
            : " ";
    ...
    function axStaticText(node: AccessibilityTreeNode): string | undefined {
      return normalize(node.name);
    }

    function axGeneratedMetadataText(node: AccessibilityTreeNode): string | undefined {
      const metadataElement = metadataElements[metadataIndex] as any;
      if (!metadataElement) return undefined;
      if (
        normalize(node.domNodeId) &&
        normalize(metadataElement.getAttribute("data-sr-dom-node-id")) &&
        normalize(node.domNodeId) !== normalize(metadataElement.getAttribute("data-sr-dom-node-id"))
      ) {
        return undefined;
      }

      const fragments: string[] = [];
      function collect(current?: AccessibilityTreeNode): void {
        if (!current) return;
        if (current.ignored) {
          for (const child of axRawChildNodes(current)) collect(child);
          return;
        }
        if (normalizedAxRole(current.role) === "statictext") {
          const text = normalize(current.name);
          if (text) fragments.push(text);
          return;
        }
        for (const child of axRawChildNodes(current)) collect(child);
      ...
      return text && expected && text === expected ? text : undefined;
    }
    ...
    function pushAxContent(node: AccessibilityTreeNode): void {
      ...
      if (role === "link") {
        flushMetadataText();
        const link = directLinks[linkIndex] as any;
        const linkName = normalizeAnnouncementLabel(node.name) || accessibleName(link, "link");
        if (
          !link ||
          !linkName ||
      ...
      if (role === "strong" || role === "emphasis") {
        ...
        if (
          ...
          axRawChildNodes(node).some((child) => normalizedAxRole(child.role) === "linebreak")
        ) {
  ...
  function axNativeMarkerListItemFragmentAnnouncements(el: any): string[] | undefined {
    const simpleInlineStrongAnnouncements =
      axSimpleNativeUlInlineStrongListItemAnnouncements(el);
    if (simpleInlineStrongAnnouncements) return simpleInlineStrongAnnouncements;

    const inlineBoundaryAnnouncements =
      axNativeInlineChildBoundaryListItemAnnouncements(el);
    ...
    if (
      Array.from(el.querySelectorAll("button, [role='button'], input, select, textarea")).some(
        (control: any) =>
          !isHidden(control) &&
          control.closest("li,[role='listitem']") === el &&
          control.closest("ul, ol, dl, [role='list']") === list,
      )
    ) {
      return undefined;
    }
  ...
  function computeAxSimpleNativeUlInlineStrongListItemAnnouncements(
    ...
    if (
      el.querySelector(
        "a[href], [role='link'], button, [role='button'], input, select, textarea, ul, ol, dl, [role='list'], [role='table'], [role='grid'], [role='treegrid']",
      )
    ) {
      return undefined;
    }
    ...
    const list = listForItem(el);
    if (!list || list.tagName?.toLowerCase() !== "ul") return undefined;
    if (list.getAttribute("role") && list.getAttribute("role") !== "list") return undefined;
    if (list.closest("nav,[role='navigation']")) return undefined;
    if ((listLevel(list) || 1) !== 1) return undefined;
    if (isMarkerSeparatedLinkList(list)) return undefined;
    ...
    const directEmphasis = Array.from(el.children || []).filter(
      (child: any) => !isHidden(child) && ["strong", "b"].includes(child.tagName?.toLowerCase()),
    );
    if (directEmphasis.length < 2) return undefined;
    if (
      Array.from(el.children || []).some((child: any) => {
        if (isHidden(child)) return true;
        const tag = child.tagName?.toLowerCase();
        if (!["strong", "b"].includes(tag)) return true;
        if (child.getAttribute("role") || child.hasAttribute("tabindex") || child.tabIndex >= 0) {
          return true;
        }
        if (
          child.querySelector?.(
            `${interactiveSelector}, [aria-hidden='true'], [hidden], [tabindex], ul, ol, dl, table`,
          )
        ) {
          return true;
        }
        return Array.from(child.children || []).some((grandchild: any) => !isHidden(grandchild));
      })
    ) {
      return undefined;
    }
    ...
    const leadingText = normalize(firstNode.name);
    if (!leadingText || leadingText !== directLeadingText(el)) return undefined;

    const fragments: string[] = [leadingText];
    const comparableFragments: string[] = [leadingText];
    let emphasisIndex = 0;
    let strongBoundaryCount = 0;

    function pushStaticText(node: AccessibilityTreeNode): boolean {
      const text = normalize(node.name);
      if (!text) return true;
      if (!/[\p{L}\p{N}]/u.test(text)) return true;
      comparableFragments.push(text);
      if (/^or$/iu.test(text)) return true;
      fragments.push(/^or\s+by$/iu.test(text) ? text.replace(/\s+/g, "") : text);
      return true;
    }

    function pushEmphasis(node: AccessibilityTreeNode): boolean {
      if (normalizedAxRole(node.role) !== "strong") return false;

      const element = directEmphasis[emphasisIndex] as any;
      ...
      if (
        children.length !== 1 ||
        normalizedAxRole(children[0].role) !== "statictext"
      ) {
        return false;
      }
      const text = normalize(children[0].name);
      if (!text || text !== normalize(readableText(element))) return false;
      fragments.push(text);
      comparableFragments.push(text);
      emphasisIndex += 1;
    ...
    for (const node of followingNodes) {
      const role = normalizedAxRole(node.role);
      if (role === "statictext") {
        if (!pushStaticText(node)) return undefined;
        continue;
      }
      if (!pushEmphasis(node)) return undefined;
    }

    if (emphasisIndex !== directEmphasis.length || strongBoundaryCount < 2) return undefined;
  ...
  function axOrderedParentNestedListItemAnnouncements(el: any): string[] | undefined {
    if (!isListItem(el) || el.tagName?.toLowerCase() !== "li") return undefined;
    if (el.getAttribute("data-sr-marker-content") !== "normal") return undefined;
    const markerDisplay = normalize(el.getAttribute("data-sr-marker-display"));
    if (markerDisplay !== "inline" && markerDisplay !== "inline-block") return undefined;
    ...
    const list = listForItem(el);
    if (!list || list.tagName?.toLowerCase() !== "ol") return undefined;
    if (list.closest("nav,[role='navigation']")) return undefined;
    if (isMarkerSeparatedLinkList(list)) return undefined;

    const directChildren = Array.from(el.children || []).filter((child: any) => !isHidden(child));
    const directNestedLists = directChildren.filter((child: any) => implicitRole(child) === "list");
    if (directNestedLists.length !== 1 || directChildren.length !== 1) return undefined;
    if (el.querySelector("a[href], [role='link'], button, [role='button']")) return undefined;
    ...
    if (normalize(textNode.name) !== text) return undefined;
    if (normalizedAxRole(listNode.role) !== "list") return undefined;
    if (
      normalize(listNode.domNodeId) !==
      normalize(directNestedLists[0].getAttribute("data-sr-dom-node-id"))
    ) {
      return undefined;
    }
  ...
  function axOrderedNestedListItemMarkerAnnouncement(el: any): string | undefined {
    if (!isListItem(el) || el.tagName?.toLowerCase() !== "li") return undefined;
    if (el.getAttribute("data-sr-marker-content") !== "normal") return undefined;
    const markerDisplay = normalize(el.getAttribute("data-sr-marker-display"));
    if (markerDisplay !== "inline" && markerDisplay !== "inline-block") return undefined;
    ...
    if (!axText || axText !== text) return undefined;
  ...
  function axPlainTextMarkerTextOnlyListItemText(el: any): string | undefined {
    ...
    if (normalizedAxRole(textNode.role) !== "statictext") return undefined;
    const marker = nativeAxListMarkerText(markerNode);
    if (!marker || (marker !== "•" && !/^\d+[.)]$/u.test(marker))) return undefined;
  ...
  function axPlainTextMarkerListItemAnnouncement(el: any): string | undefined {
    ...
    const hasCompatibleMarkerSiblings = siblings.every(
      (item: any) =>
        ...
        axSimpleNativeUlInlineStrongListItemAnnouncements(item) ||
    ...
    const hasSimpleNativeUlInlineStrongSibling = siblings.some(
      (item: any) => item !== el && axSimpleNativeUlInlineStrongListItemAnnouncements(item),
    );
    if (
      !hasDirectLink &&
      ...
      !hasSimpleNativeUlInlineStrongSibling
    ) {
      return undefined;
    }
  ...
  function generatedParentheticalMetadataText(node: any): string | undefined {
    if (!node || node.nodeType !== Node.ELEMENT_NODE || isHidden(node)) return undefined;
    ...
    if (!text || text.length > 80 || !/[\p{L}\p{N}]/u.test(text)) return undefined;
    return `(${text})`;
  }
  ...
  function directAxInlineTextLinkParagraphFragments(el: any): string[] | undefined {
    ...
    function axNodeContainsFocusableLink(node?: AccessibilityTreeNode): boolean {
    ...
    function shouldCompactMedialInlineLinkConjunction(
      index: number,
      text: string,
      rawName?: string,
    ): boolean {
      if (links.length < 2 || directElements.length !== links.length) return false;
      if (
        !directElements.every((element: any) =>
          ["strong", "b", "em", "i"].includes(element.tagName?.toLowerCase()),
        )
      ) {
        return false;
      }
      if (!/^\s+(?:and|or)\s+[\p{L}\p{N}]/iu.test(rawName || "")) return false;
      if (!/^(?:and|or)\s+[\p{L}\p{N}]/iu.test(text)) return false;
      if (!axNodeContainsFocusableLink(axChildren[index - 1])) return false;
      if (!axNodeContainsFocusableLink(axChildren[index + 1])) return false;
      return true;
    }
  ...
  function directLinkGeneratedMetadataFragments(el: any): string[] | undefined {
    if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el)) return undefined;
    const tag = el.tagName.toLowerCase();
    if (tag !== "p" && tag !== "small") return undefined;
    if (
    ...
    function appendToMetadata(value?: string): void {
      if (!value) return;
      const text = normalize(value);
      if (!text) return;
      metadataText = metadataText && needsBoundary(metadataText, text)
        ? `${metadataText} ${text}`
        : `${metadataText}${metadataText ? " " : ""}${text}`;
    }
  ...
  function directAxInlineLinkBreakParagraphFragments(el: any): string[] | undefined {
    ...
    const hasDirectStaticTextLinkStaticTextShape =
      ...
      normalizedAxRole(axChildren[1]?.role) === "link" &&
      normalizedAxRole(axChildren[2]?.role) === "statictext";
    const hasDirectStaticTextTwoLinkStaticTextShape =
      directLinks.length === 2 &&
      directElements.length === 2 &&
      directElements.every((element: any, index: number) => element === directLinks[index]) &&
      axChildren.length === 5 &&
      normalizedAxRole(axChildren[0]?.role) === "statictext" &&
      normalizedAxRole(axChildren[1]?.role) === "link" &&
      normalizedAxRole(axChildren[2]?.role) === "statictext" &&
      normalizedAxRole(axChildren[3]?.role) === "link" &&
      normalizedAxRole(axChildren[4]?.role) === "statictext" &&
      Boolean(normalize(axChildren[0]?.name)?.match(/[\p{L}\p{N}]/u)) &&
      Boolean(normalize(axChildren[2]?.name)?.match(/^(?:and|or|&)$/iu)) &&
      Boolean(normalize(axChildren[4]?.name)?.match(/[\p{L}\p{N}]/u));
    ...
    const canUseNoBreakStaticTextTwoLinkStaticTextShape =
      hasDirectStaticTextTwoLinkStaticTextShape &&
      !el.closest("aside,footer,nav");
    if (
      !hasAxLineBreak &&
      !canUseNoBreakStaticTextLinkStaticTextShape &&
      !canUseNoBreakStaticTextTwoLinkStaticTextShape
    ) {
      return undefined;
    }
  ...
  function compactFocusableTokenizedPreCodeText(pre: any): string | undefined {
    if (!pre || pre.nodeType !== Node.ELEMENT_NODE || isHidden(pre)) return undefined;
    if (pre.tagName?.toLowerCase() !== "pre") return undefined;
    const tabindex = normalize(pre.getAttribute("tabindex"));
    if (tabindex === undefined || tabindex === "-1") return undefined;

    const code = directPreCodeChild(pre);
    if (!code || isHidden(code) || !hasSyntaxHighlightedCodeDescendants(code)) return undefined;
    if (tokenizedPreCodeLines(pre) || tokenizedMixedHtmlFormFragments(pre)) return undefined;

    const text = normalize(readableText(code) || code.textContent)?.replace(/([^\s{])\}/gu, "$1 }");
    if (!text || text.length < 8 || /^</u.test(text)) return undefined;
    return `${text} group`;
  }
  ...
  function directHeadingFragments(el: any): string[] | undefined {
    ...
    function axHeadingStaticTextBoundaryFragments(): string[] | undefined {
      ...
      const tag = el.tagName?.toLowerCase();
      const isNativeHeading = /^h[1-6]$/u.test(tag || "");
      const fragments: string[] = [];
      ...
      function collectStaticText(
        node?: AccessibilityTreeNode,
        options: { direct?: boolean; previousDirectRole?: string; nextDirectRole?: string } = {},
      ): boolean {
        if (!node) return false;
        const role = normalizedAxRole(node.role);
        if (role === "statictext") {
          ...
          if (fragment && /[\p{L}\p{N}$£€]/u.test(fragment)) {
            ...
            const isOrdinarySpaceChild = /^ +$/u.test(rawName) &&
              options.previousDirectRole === "statictext" &&
              options.nextDirectRole === "statictext";
            const isTrailingNbspAfterLineBreak = /^[\u00A0]+$/u.test(rawName) &&
              options.previousDirectRole === "linebreak";
      ...
      const childNodes = childIds.map((childId) =>
        accessibilityNodeById.get(normalize(childId) || ""),
      );
      if (
        !childNodes.every((childNode, index) =>
          collectStaticText(childNode, {
            direct: true,
      ...
      if (!fragmentsAppearInTextOrder(textFragments, axName)) return undefined;
      if (normalize(textFragments.join(" ")) !== axName) return undefined;
      return fragments;
    }
  ...
  function captureElement(el: any): CapturedElementDescriptor | null {
    ...
    const axNativeButtonSymbolSpacingName =
      role === "button"
        ? axConfirmedNativeButtonSymbolSpacingName(el, role, axWhitespaceButtonName || name)
        : undefined;
    ...
    const suppressSingleButtonNestedGenericGroup =
      isAxConfirmedSingleButtonNestedGenericGroupControl(el, role, name);
    ...
    const descriptor: CapturedElementDescriptor = {
      ...
      suppressAutocomplete:
        ...
        isTextboxYieldedFromAxComboboxWrapper(el, role) ||
        undefined,
      ...
      fieldsetLegendText:
        role === "group"
          ...
          : undefined,
      standaloneLabelTextGroupText:
        role === "group" && isAxConfirmedStandaloneLabelTextGroup(el)
          ? normalize(textWithoutInteractive(el) || readableText(el))
          : undefined,
      ...
      axOrderedParentNestedListItemAnnouncements:
        role === "listitem" ? axOrderedParentNestedListItemAnnouncements(el) : undefined,
      axOrderedNestedListItemMarkerAnnouncement:
        role === "listitem" ? axOrderedNestedListItemMarkerAnnouncement(el) : undefined,
      axOrderedBlockChildListItemMarkerAnnouncement:
        role === "listitem" ? axOrderedBlockChildListItemMarkerAnnouncement(el) : undefined,
      axOrderedDirectTextListItemAnnouncements:
        role === "listitem" ? axOrderedDirectTextListItemAnnouncements(el) : undefined,
      axNativeMarkerListItemFragmentAnnouncements:
        role === "listitem" ? axNativeMarkerListItemFragmentAnnouncements(el) : undefined,
  ...
  function isFooterLegalControl(el: any): boolean {
    const role = implicitRole(el);
    if (role === "link") return el.tagName?.toLowerCase() === "a" && el.hasAttribute("href");
    if (role === "button") return el.tagName?.toLowerCase() === "button";
    const links = Array.from(el?.querySelectorAll?.("a[href], [role='link']") || []).filter(
      (link: any) => !isHidden(link),
    );
    if (links.length >= 2 && !Array.from(el.querySelectorAll?.("button") || []).some((button: any) => !isHidden(button))) {
      return true;
    }
  ...
  function footerLegalActionWrapper(el: any): any {
    for (let current = el; current; current = current.parentElement) {
      const parent = current.parentElement;
      if (!parent || parent === document.body || parent === document.documentElement) break;
      if (!parent.closest?.("footer,[role='contentinfo']")) break;
      if (previousVisibleElementSibling(current)) return current;
      if (current.tagName?.toLowerCase() === "footer") break;
    }
    return el;
  }

  function isFooterLegalPipeSeparatorText(el: any, role = implicitRole(el)): boolean {
    if (role !== "text") return false;
    if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el)) return false;
    if (!el.closest("footer,[role='contentinfo']")) return false;
    if (el.matches(interactiveSelector) || el.querySelector(interactiveSelector)) return false;
    if (!["span", "small", "div"].includes(el.tagName?.toLowerCase())) return false;
    if (!/^\|$/u.test(normalize(readableText(el) || el.textContent) || "")) return false;
    ...
    if (!isFooterLegalControl(previous) || !isFooterLegalControl(next)) return false;

    const footer = closestFooterContext(el);
    const links = Array.from(footer?.querySelectorAll?.("a[href], [role='link']") || []).filter(
      (link: any) => !isHidden(link),
    );
    const separators = Array.from(footer?.querySelectorAll?.("span, small, div") || []).filter(
      (candidate: any) =>
        candidate !== el &&
        !isHidden(candidate) &&
        /^\|$/u.test(normalize(readableText(candidate) || candidate.textContent) || ""),
    );
    return links.length >= 2 && separators.length >= 1;
  }

  function isFooterLegalActionButton(el: any, role = implicitRole(el)): boolean {
    if (!nativeButtonSingleTextAndDecorativeMedia(el, role)) return false;
    if (!el.closest("footer,[role='contentinfo']")) return false;
    if (el.hasAttribute("aria-label") || el.hasAttribute("aria-labelledby")) return false;
    ...
    if (!previous || !isFooterLegalPipeSeparatorText(previous)) return false;
    const beforeSeparator = previousVisibleElementSibling(previous);
    if (!beforeSeparator || !isFooterLegalControl(beforeSeparator)) return false;

    const footer = closestFooterContext(el);
    const legalLinks = Array.from(footer?.querySelectorAll?.("a[href], [role='link']") || []).filter(
      (link: any) => !isHidden(link),
    );
  ...
  function isPaginationNavigationControl(el: any, role = implicitRole(el)): boolean {
    ...
    if (accessibilityNodes.length) {
      const axNode = axNodeForElementRole(el, role);
      if (!axNode || normalize(axNode.name) !== controlName) return false;
    }

    return true;
  }

  function isAxExposedAnonymousSvgImage(el: any, role = implicitRole(el)): boolean {
    if (role !== "image") return false;
    if (el?.tagName?.toLowerCase() !== "svg") return false;
  ...
  function hasAxExposedAnonymousSvgListItemContent(el: any, role = implicitRole(el)): boolean {
    if (role !== "listitem") return false;
    if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el)) return false;
    if (hasVisibleInteractiveDescendant(el)) return false;
    if (!normalize(readableText(el))) return false;

    const list = el.parentElement;
    if (!list || implicitRole(list) !== "list") return false;
    if (!positionInSet(el, "listitem") || !setSize(el, "listitem")) return false;

    const svgs = Array.from(el.querySelectorAll("svg")).filter((svg: any) =>
      isAxExposedAnonymousSvgImage(svg, "image"),
    );
    if (svgs.length !== 1) return false;

    const textLeaves = Array.from(el.querySelectorAll("span, div")).filter(
      (candidate: any) =>
        !isHidden(candidate) &&
        !candidate.querySelector("svg, img, button, a[href], input, select, textarea") &&
        Boolean(normalize(directOwnText(candidate))),
    );
    return textLeaves.length === 1;
  }

  function isAxExposedAnonymousSvgListItemImage(el: any, role = implicitRole(el)): boolean {
    if (!isAxExposedAnonymousSvgImage(el, role)) return false;
    const listItem = el.closest?.("li,[role='listitem']");
    return Boolean(listItem && hasAxExposedAnonymousSvgListItemContent(listItem, "listitem"));
  }

  function isAxExposedAnonymousSvgFormIcon(el: any, role = implicitRole(el)): boolean {
    if (!isAxExposedAnonymousSvgImage(el, role)) return false;
    ...
    const control = controls[0] as any;
    if (control.closest?.("a[href], button, [role='button'], [role='link']")) return false;
    return field.contains(control) && field.contains(el);
  }

  function isAxConfirmedEmptyNamedGenericGroup(el: any): boolean {
    if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el)) return false;
    const tag = el.tagName?.toLowerCase();
    if (tag !== "span" && tag !== "div") return false;
    if (el.matches(interactiveSelector) || hasVisibleInteractiveDescendant(el)) return false;
    if (normalize(readableText(el) || el.textContent)) return false;

    const name = explicitAriaName(el);
    if (!name) return false;

    const siblings = Array.from(el.parentElement?.children || []);
    const controls = siblings.filter(
      (sibling: any) =>
        sibling !== el &&
        sibling.matches?.("input:not([type='hidden']), select, textarea") &&
        !isHidden(sibling),
    );
    if (controls.length !== 1) return false;
    const formIcons = siblings
      .filter((sibling: any) => sibling !== el)
      .flatMap((sibling: any) =>
        sibling.tagName?.toLowerCase() === "svg"
          ? [sibling]
          : Array.from(sibling.querySelectorAll?.("svg") || []),
      )
      .filter((svg: any) => isAxExposedAnonymousSvgImage(svg, "image"));
    if (formIcons.length !== 1) return false;
  ...
  function isAxConfirmedSingleButtonNestedGenericGroupControl(
    ...
    const parent = el.parentElement;
    if (!parent || implicitRole(parent) !== "group") return false;
    const parentName = normalize(accessibleName(parent, "group"));
    if (!parentName) return false;

    const visibleChildren = Array.from(parent.children || []).filter((child: any) => !isHidden(child));
    if (visibleChildren.length !== 1 || visibleChildren[0] !== el) return false;

    const parentNode = axNodeForElementRole(parent, "generic");
    const buttonNode = axNodeForElementRole(el, "button");
    if (!parentNode || !buttonNode) return false;
    if (normalize(parentNode.name) !== parentName) return false;
    if (normalize(buttonNode.name) !== normalize(name || accessibleName(el, role))) return false;
    if (buttonNode.properties?.focusable !== true) return false;

    const childIds = parentNode.childIds || [];
    return childIds.length === 1 && normalize(childIds[0]) === normalize(buttonNode.nodeId);
  }

  function isAxConfirmedComboboxWrapperYieldingToTextbox(el: any): boolean {
    if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el)) return false;
    if (el.getAttribute("role") !== "combobox") return false;
    if (el.getAttribute("aria-label") || el.getAttribute("aria-labelledby")) return false;
    if (el.matches("input, select, textarea")) return false;

    const inputs = Array.from(el.querySelectorAll("input")).filter((input: any) => {
    ...
    if (!normalize(input.getAttribute("placeholder")) && !normalize(input.value)) return false;

    const wrapperNode = axNodeForElementRole(el, "combobox");
    const textboxNode = axNodeForElementRole(input, "textbox");
    if (!wrapperNode || !textboxNode) return false;
    if (wrapperNode.properties?.focusable === true) return false;
    if (textboxNode.properties?.focusable !== true) return false;
  ...
  function isTextboxYieldedFromAxComboboxWrapper(el: any, role = implicitRole(el)): boolean {
    if (role !== "textbox") return false;
    const wrapper = el?.parentElement;
    return Boolean(wrapper && isAxConfirmedComboboxWrapperYieldingToTextbox(wrapper));
  }

  function isAxConfirmedStandaloneLabelTextGroup(el: any): boolean {
    if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el)) return false;
    if (el.tagName?.toLowerCase() !== "label") return false;
    if (hasVisibleInteractiveDescendant(el)) return false;
    ...
    const visibleText = normalize(textWithoutInteractive(el) || readableText(el));
    if (!labelName || !visibleText) return false;

    const controlId = normalize(el.getAttribute("for"));
    if (controlId) return false;

    const axNode = axNodeForElementRole(el, "labeltext");
    if (!axNode || normalize(axNode.name) !== labelName) return false;
    return axDescendants(axNode).some(
      (node) =>
        !node.ignored &&
        normalizedAxRole(node.role) === "statictext" &&
        normalize(node.name) === visibleText,
    );
  }

  function axWhitespaceAdjustedButtonName(
    el: any,
    role: string,
    name?: string,
  ): string | undefined {
    if (role !== "button" || !name) return undefined;
    if (el?.tagName?.toLowerCase() !== "button") return undefined;
    if (el.hasAttribute("aria-label") || el.hasAttribute("aria-labelledby")) return undefined;
    ...
    const compact = (value: string) => normalize(value)?.replace(/\s+/g, "");
    if (compact(axName) !== compact(name)) return undefined;
  ...
  function axPoliteRegionText(el: any, role = implicitRole(el)): string | undefined {
    if (role !== "region") return undefined;
    if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el)) return undefined;
    if (el.getAttribute("role") !== "region") return undefined;
    const tag = el.tagName?.toLowerCase();
    if (tag !== "div" && tag !== "section") return undefined;
    if (normalize(accessibleName(el, role))) return undefined;
    if (normalize(el.getAttribute("aria-live")) !== "polite") return undefined;
    if (hasVisibleInteractiveDescendant(el)) return undefined;
    ...
    const staticText = descendants
      .filter((node) => normalizedAxRole(node.role) === "statictext")
      .map((node) => normalize(node.name))
      .filter((fragment): fragment is string => Boolean(fragment));
    return normalize(staticText.join(" ")) === text ? text : undefined;
  }

  function isUnnamedAxPoliteLiveRegion(el: any, role = implicitRole(el)): boolean {
    if (role !== "region") return false;
    if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el)) return false;
    if (normalize(accessibleName(el, role))) return false;
    if (normalize(el.getAttribute("aria-live")) !== "polite") return false;
  ...
  function isStopElement(el: any): boolean {
    ...
    if (
      ...
      !isAxExposedAnonymousSvgListItemImage(el, role) &&
      !isAxExposedAnonymousSvgFormIcon(el, role) &&
  ...
  function shouldDescendIntoStop(el: any): boolean {
    ...
    if (role === "listitem") {
      if (axOrderedParentNestedListItemAnnouncements(el)) return true;
      if (axOrderedNestedListItemMarkerAnnouncement(el)) return false;
      if (axOrderedBlockChildListItemMarkerAnnouncement(el)) return true;
      ...
      if (hasAxExposedAnonymousSvgListItemContent(el, role)) return true;
      if (
  ...
  function splitStandaloneLabelTextGroupAnnouncements(
    descriptor: CapturedElementDescriptor,
  ): string[] | undefined {
    const labelText = normalize(descriptor.standaloneLabelTextGroupText);
    if (descriptor.role !== "group" || !labelText) return undefined;
    return [generateAnnouncement(descriptor), labelText];
  }
  ...
  function splitAxNativeMarkerListItemFragmentAnnouncements(
    descriptor: CapturedElementDescriptor,
  ): string[] | undefined {
    if (
      descriptor.role !== "listitem" ||
  ...
  function splitAxOrderedParentNestedListItemAnnouncements(
    ...
    if (
      descriptor.role !== "listitem" ||
      !descriptor.axOrderedParentNestedListItemAnnouncements?.length
    ) {
      return undefined;
    }
  ...
  function splitAxOrderedNestedListItemMarkerAnnouncements(
    descriptor: CapturedElementDescriptor,
  ): string[] | undefined {
    if (
      descriptor.role !== "listitem" ||
      !descriptor.axOrderedNestedListItemMarkerAnnouncement
    ) {
      return undefined;
    }
  ...
  function splitAxOrderedBlockChildListItemMarkerAnnouncements(
    descriptor: CapturedElementDescriptor,
  ): string[] | undefined {
    if (
      descriptor.role !== "listitem" ||
      !descriptor.axOrderedBlockChildListItemMarkerAnnouncement
    ) {
      return undefined;
    }
  ...
  function splitAxOrderedDirectTextListItemAnnouncements(
    descriptor: CapturedElementDescriptor,
  ): string[] | undefined {
    if (
      descriptor.role !== "listitem" ||
      !descriptor.axOrderedDirectTextListItemAnnouncements?.length
    ) {
      return undefined;
    }
  ...
  function scanSubtree(root: any): ScanLogEntry[] {
    ...
    function walk(el: any): void {
      ...
      if (compactFocusableCodeText) {
        ...
        emitTraversalStop(
          id,
          syntheticTextStop("compact-focusable-tokenized-pre-code", el, compactFocusableCodeText),
        );
        return;
      }
      ...
      if (politeRegionText) {
        ...
        emitTraversalStop(
          id,
          syntheticTextStop("ax-polite-region-text", el, politeRegionText),
        );
        return;
      }
      ...
      if (isStopElement(el)) {
        ...
        if (shouldDescendIntoStop(el)) {
          const children = descriptor?.modalDialogSummaryItemCount
            ...
            : descriptor?.axOrderedParentNestedListItemAnnouncements
