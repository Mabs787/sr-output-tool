  ...
  var require_announcements = __commonJS({
    "../sr-engine/dist/announcements.js"(exports) {
      ...
      function formatHeadingFragments(level, fragments, fragmentCount) {
        const normalizedFragments = fragments?.map((fragment) => normalizeHeadingFragment(fragment)).filter((fragment) => Boolean(fragment));
          ...
          case "combobox": {
            ...
            if (el.nativeSelect) {
              ...
              if (el.nativeDatalistPlaceholderName && comboLabel && popupType) {
                ...
                if (popupType && el.expanded !== void 0) {
                  parts.push(`${el.required ? "required " : ""}${popupType} ${el.expanded ? "expanded" : "collapsed"}`);
                } else if (popupType) {
    ...
    "../sr-engine/dist/dom.js"(exports) {
      ...
      function createDomScanner2(options) {
        ...
        function isAxConfirmedNativeCollapsedButtonWithHiddenControlledRegions(el, role, name) {
          ...
          if (!buttonName)
            return false;
          if (!hasHiddenControlledRegionsForCollapsedNativeButton(el, role))
            return false;
          const axNode = axNodeForElementRole(el, "button");
        ...
        function hiddenControlledRegionsFor(el) {
          const controls = normalize(el?.getAttribute?.("aria-controls"));
          if (!controls)
            return [];
          return controls.split(/\s+/).map((id) => resolveIdRefFrom(el, id)).filter(Boolean);
        }
        function hasHiddenControlledRegionsForCollapsedNativeButton(el, role) {
          if (role !== "button")
            return false;
          if (el?.tagName?.toLowerCase() !== "button")
            return false;
        ...
        function unassignedCollapsedDisclosureSummarySlotChildren(host, slotName) {
          if (slotName.toLowerCase() !== "summary")
            return [];
          if (!isCustomElement(host) || !hasSerializedShadowRootContent(host))
            return [];
          const hasCollapsedSummarySlot = (node) => {
            if (!node || node.nodeType !== Node.ELEMENT_NODE)
              return false;
            const tag = node.tagName?.toLowerCase();
            if (/^h[1-6]$/.test(tag || "") && Array.from(node.children || []).some((child) => {
              if (child.tagName?.toLowerCase() !== "button")
                return false;
              if (parseBooleanAttribute(child, "aria-expanded") !== false)
                return false;
              if (!child.hasAttribute("aria-controls"))
                return false;
              return Array.from(child.children || []).some((grandchild) => grandchild.tagName?.toLowerCase() === "slot" && (grandchild.getAttribute("name") || "").toLowerCase() === "summary");
            })) {
              return true;
            }
            return Array.from(node.children || []).some((child) => hasCollapsedSummarySlot(child));
          };
          if (!shadowContentChildren(host).some((child) => hasCollapsedSummarySlot(child))) {
            return [];
          }
        ...
        function assignedSlotNodes(slot) {
          ...
          const disclosureSummarySlotChildren = unassignedCollapsedDisclosureSummarySlotChildren(host, slotName);
          if (disclosureSummarySlotChildren.length)
            return disclosureSummarySlotChildren;
        ...
        function localFooterNavigationLabelledListLabel(list) {
          ...
          const visibleChildren = Array.from(list.children || []).filter((child) => !isHidden(child));
          if (visibleChildren.length < 2)
            return void 0;
          const labelledIds = new Set(labelledBy.split(/\s+/).filter(Boolean));
          const label = visibleChildren[0];
          const labelRole = normalize(label.getAttribute?.("role"))?.toLowerCase();
          const labelId = normalize(label.getAttribute?.("id"));
          if (label.tagName?.toLowerCase() !== "li")
            return void 0;
          if (labelRole !== "none" && labelRole !== "presentation")
            return void 0;
          if (!labelId || !labelledIds.has(labelId))
            return void 0;
          if (label.querySelector?.(interactiveSelector))
            return void 0;
          if (Array.from(label.children || []).some((child) => !isHidden(child)))
            return void 0;
          const labelText = normalize(readableText(label) || label.textContent);
          if (!labelText)
            return void 0;
          const hasDuplicateLabelId = Array.from(document.querySelectorAll(`#${cssEscape(labelId)}`)).filter((candidate) => normalize(candidate.getAttribute?.("id")) === labelId).length > 1;
          const linkItems = visibleChildren.slice(1).filter((child) => {
            if (child.tagName?.toLowerCase() !== "li")
              return false;
            const role = normalize(child.getAttribute?.("role"))?.toLowerCase();
            if (role && role !== "listitem")
              return false;
            return Array.from(child.querySelectorAll?.("a[href], [role='link']") || []).some((link) => !isHidden(link));
          });
          if (!linkItems.length)
            return void 0;
          ...
          if (hasDuplicateLabelId || accessibilityNodes.length && hasMappedAxCandidate) {
            ...
            if (!listAxNode)
              return void 0;
            if (normalize(listAxNode.name) !== labelText)
              return void 0;
            if ((listAxNode.childIds || []).length < linkItems.length + 1)
              return void 0;
          }
          return label;
        }
        function localFooterNavigationLabelledListLabelName(list, role) {
          if (role !== "list")
            return void 0;
          const label = localFooterNavigationLabelledListLabel(list);
          return label ? normalize(readableText(label) || label.textContent) : void 0;
        }
        function isFooterNavigationLabelledListLabelItem(el) {
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
        ...
        function isEmptyHeadingBeforeContentBoundary(el, role) {
          if (role !== "heading")
            return false;
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
          if (el.matches?.(interactiveSelector) || el.hasAttribute("tabindex"))
            return false;
        ...
        function isExposedUnnamedMediaElement(el) {
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
          const role = normalize(el.getAttribute("role"))?.toLowerCase();
        ...
        function isNativeAriaLabelButtonWithExposedMediaChild(el, role = implicitRole(el)) {
          if (!isPlainNativeButton(el, role))
            return false;
          if (!el.hasAttribute("aria-label"))
            return false;
          if (readableText(el) || hasNonTooltipDescendantText(el))
            return false;
          if (nestedImageLabel(el))
            return false;
          const visibleChildren = Array.from(el.children || []).filter((child) => !isHidden(child));
          if (!visibleChildren.length)
            return false;
          return visibleChildren.every((child) => isExposedUnnamedMediaElement(child));
        }
        ...
        function accessibleName(el, role) {
          ...
          if (ariaLabel !== void 0)
            return ariaLabel;
          if (localFooterNavigationListLabel)
            return localFooterNavigationListLabel;
        ...
        function implicitRole(el) {
          ...
          if (explicit === "img")
            return "image";
          if (isFooterNavigationLabelledListLabelItem(el))
            return "listitem";
          if (hasPresentationRole(el)) {
        ...
        function customHeadedTextCardFor(el) {
          ...
          for (let current = el?.parentElement, depth = 0; current && depth < 8; current = current.parentElement, depth += 1) {
            ...
            if (!(heading.compareDocumentPosition(el) & documentNode.DOCUMENT_POSITION_FOLLOWING)) {
              continue;
            }
        ...
        function expandedRegionInlineLinkFragments(el) {
          ...
          if (!expandedControlledRegionFor(el))
            return void 0;
          const links = Array.from(el.querySelectorAll("a[href], [role='link'], [role='button']")).filter((link2) => !isHidden(link2) && ["link", "button"].includes(implicitRole(link2)));
          if (links.length !== 1)
            return void 0;
          const link = links[0];
        ...
        function isCollapsedDisclosureHeadingWithHiddenControlledRegion(el, role = implicitRole(el)) {
          if (role !== "heading")
            return false;
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
          const visibleChildren = walkChildren(el).filter((child) => !isHidden(child) && !child.matches?.("script, style, template"));
          if (visibleChildren.length !== 1)
            return false;
          const button = visibleChildren[0];
          if (implicitRole(button) !== "button")
        ...
        function wrapsOnlyNativeExposedLinkButtonOrGroupStops(el) {
          ...
          const visit = (node) => {
            ...
            if (node !== el) {
              const role = implicitRole(node);
              if (isNativeExposedLinkButtonOrGroup(node, role) || isNamedCustomShadowGroupStop(node, role) || isCollapsedDisclosureHeadingWithHiddenControlledRegion(node, role)) {
        ...
        function closestAnonymousCustomMediaWrapper(el) {
          const host = closestCustomElement(el);
          if (!host || host.matches?.(interactiveSelector))
            return null;
          if (customElementContributesLabelRoleOrState(host))
            return null;
          if (accessibleName(host, "group"))
            return null;
        ...
        function isPlainButtonInsideAnonymousCustomMediaWrapper(el, role) {
          if (role !== "button")
            return false;
          if (el?.tagName?.toLowerCase() !== "button")
            return false;
        ...
        function directHeadingFragments(el) {
          ...
          if (el.querySelector("button, [role='button'], a[href]"))
            return void 0;
          function normalizeHeadingTextFragment(value, options2 = {}) {
            if (!value)
              return void 0;
            if (value === "space ")
              return value;
            if (/^[\s\u00A0]+$/u.test(value)) {
              return options2.preserveTrailingSpace ? "space " : "space";
            }
            return normalize(value);
          }
          function comparableHeadingFragmentsText(fragments2) {
            return normalize(fragments2.map((fragment) => /^space\s*$/u.test(fragment) ? " " : fragment).join(""));
          }
          function axHeadingChildFragments() {
            if (!accessibilityNodes.length)
              return void 0;
            if (Array.from(el.children || []).some((child) => !isHidden(child) && Boolean(parenthesizedBoundaryPartCount(child)))) {
              return void 0;
            }
            const axNode = axNodeForElementRole(el, "heading");
            const axName = normalize(axNode?.name);
            ...
            if (childIds.length < 2)
              return void 0;
            function childNodesIncludingIgnored(node) {
            ...
            function collectNodeText(node) {
              ...
              const parts = childNodesIncludingIgnored(node).map((child) => collectNodeText(child)).filter((part) => part !== void 0 && part !== "");
              return parts.length ? parts.join("") : void 0;
            }
            const axChildren = childNodesIncludingIgnored(axNode);
            const fragmentEntries = axChildren.map((child) => ({ child, text: collectNodeText(child) })).filter((entry) => entry.text !== void 0 && entry.text !== "");
            const rawFragments = fragmentEntries.map((entry) => entry.text);
            if (rawFragments.length < 2)
              return void 0;
            if (rawFragments.some((fragment) => fragment === "\n"))
              return void 0;
            const fragments2 = fragmentEntries.map((entry, index) => {
              ...
              return normalizeHeadingTextFragment(entry.text, {
                preserveTrailingSpace: /^[\s\u00A0]+$/u.test(entry.text) && ["strong", "emphasis"].includes(previousRole || "") && Boolean(normalize(nextText))
              });
            }).filter((fragment) => Boolean(fragment));
            if (fragments2.length < 2)
              return void 0;
            if (fragments2.filter((fragment) => !/^space\s*$/u.test(fragment)).length < 2) {
              return void 0;
            }
            const compactText = comparableHeadingFragmentsText(rawFragments);
            const spacedText = normalize(rawFragments.join(" "));
            if (compactText !== axName && spacedText !== axName)
              return void 0;
            return fragments2;
          }
          function domInlineHeadingFragments() {
            if (Array.from(el.children || []).some((child) => !isHidden(child) && Boolean(parenthesizedBoundaryPartCount(child)))) {
              return void 0;
            }
            const allowedInlineSelector = "span, strong, b, em, i, code";
            if (Array.from(el.querySelectorAll("*")).some((node) => {
              if (isHidden(node))
                return false;
              if (node.matches?.(interactiveSelector))
                return true;
            ...
            const rawFragments = [];
            const visibleNodes = Array.from(el.childNodes || []).filter((node) => {
              if (node.nodeType === Node.TEXT_NODE)
            ...
            for (let index = 0; index < visibleNodes.length; index += 1) {
              ...
              if (node.nodeType === Node.TEXT_NODE) {
                ...
                if (/^[\s\u00A0]+$/u.test(text2)) {
                  ...
                  if (previous?.nodeType === Node.ELEMENT_NODE && next?.nodeType === Node.ELEMENT_NODE) {
                    const previousTag = previous.tagName?.toLowerCase();
                    rawFragments.push(["strong", "b", "em", "i"].includes(previousTag) ? "space " : text2);
                  }
                  continue;
                }
                rawFragments.push(text2);
                continue;
              }
              const text = readableText(node);
              if (text)
            ...
            const fragments2 = rawFragments.map((fragment) => normalizeHeadingTextFragment(fragment)).filter((fragment) => Boolean(fragment));
            if (fragments2.length < 2)
              return void 0;
            if (fragments2.filter((fragment) => !/^space\s*$/u.test(fragment)).length < 2) {
              return void 0;
          ...
          if (domInlineFragments)
            return domInlineFragments;
        ...
        function nativeSelectValue(el) {
          if (el?.tagName?.toLowerCase() !== "select")
            return void 0;
          const selectedOption = Array.from(el.options || []).find((option) => option.hasAttribute?.("selected"));
          const customHostValue = normalize(closestCustomElement(el)?.getAttribute?.("value"));
          const customHostOption = customHostValue ? Array.from(el.options || []).find((option) => normalize(option.getAttribute?.("value")) === customHostValue) : void 0;
          const selectedIndex = typeof el.selectedIndex === "number" && el.selectedIndex >= 0 ? el.selectedIndex : void 0;
          return normalize(selectedOption?.textContent) || normalize(customHostOption?.textContent) || normalize(el.selectedOptions?.[0]?.textContent) || (selectedIndex !== void 0 ? normalize(el.options?.[selectedIndex]?.textContent) : void 0) || ("value" in el && el.value ? normalize(el.value) : void 0);
        }
        ...
        function captureElement(el) {
          ...
          const suppressHeadingButtonGroup = role === "heading" && headingButton && Boolean(anonymousStructuralCustomElementHost) && hasHiddenControlledRegionsForCollapsedNativeButton(headingButton, "button");
          const suppressCustomMediaWrapperButtonGroup = role === "button" && isPlainButtonInsideAnonymousCustomMediaWrapper(el, role);
          ...
          const descriptor = {
            role,
            name: carouselControlName || visibleTextEllipsisButtonName(el, role) || normalizedFieldsetGroupName || (nativeInputComboboxPlaceholderName ? void 0 : announcementName) || nativeSelectTitleName || focusableFeedbackGroupText,
            ...
            details: carouselControlName ? void 0 : textFromIdRefsFrom(stateEl, stateEl.getAttribute("aria-describedby") ?? el.getAttribute("aria-describedby")),
            ...
            duplicateCollectionPosition: role === "listitem" && isFooterNavigationLabelledListLabelItem(el) || role === "term" && (isWrappedDefinitionListItem(el) || isSimpleDirectDefinitionListItem(el) || isDirectListBackedDefinitionItem(el) || Boolean(definitionListDisclosureButton(el))) || role === "heading" && Boolean(flattenedSlottedCarouselPosition(el).positionInSet) || void 0,
            ...
            dateTextField: role === "textbox" && tag === "input" && (el.getAttribute("type") || "text").toLowerCase() === "date" ? true : void 0,
          ...
          function walk(el) {
            ...
            if (isStopElement(el)) {
              ...
              if (shouldDescendIntoStop(el)) {
                const children = descriptor?.modalDialogSummaryItemCount ? modalDialogHeadingChildren(el) : descriptor?.nativeMarkerListItemAnnouncements ? nativeMarkerListItemDescendantLists(el) : walkChildren(el);
                for (const child of children)
                  walk(child);
              }
              if (descriptor) {
