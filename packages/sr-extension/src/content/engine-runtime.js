    ...
    "../sr-engine/dist/announcements.js"(exports) {
      ...
      function generateAnnouncement2(el) {
        ...
        switch (role) {
          ...
          case "tab": {
            pushIfPresent(parts, label);
            if (el.selected && el.tabExpandedState && el.expanded !== void 0) {
              parts.push(`selected ${el.expanded ? "expanded" : "collapsed"}`);
            } else if (el.selected) {
              parts.push("selected");
            } else if (el.tabExpandedState && el.expanded !== void 0) {
              parts.push(el.expanded ? "expanded" : "collapsed");
            }
            const popupType = formatPopupType(el.hasPopup);
            if (popupType) {
  ...
  var require_dom = __commonJS({
    "../sr-engine/dist/dom.js"(exports) {
      ...
      function createDomScanner2(options) {
        ...
        function duplicateDomIdLinks(el) {
          if (el?.tagName?.toLowerCase() !== "a" || !el.hasAttribute("href"))
            return [];
          const domNodeId = normalize(el.getAttribute("data-sr-dom-node-id"));
          if (!domNodeId)
            return [];
          return Array.from(document.querySelectorAll(`a[href][data-sr-dom-node-id="${cssEscape(domNodeId)}"]`)).filter((link) => !isHidden(link));
        }
        function axDuplicateDomIdLinkName(el) {
          const links = duplicateDomIdLinks(el);
          if (links.length < 2 || links[0] !== el)
            return void 0;
          if (!links.every((link) => normalize(link.getAttribute("href")) === normalize(el.getAttribute("href")))) {
            return void 0;
          }
        ...
        function isDuplicateDomIdAxNamedLinkRemainder(el) {
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
          if (!["div", "span"].includes(el.tagName?.toLowerCase()))
            return false;
          const link = Array.from(el.querySelectorAll("a[href][data-sr-dom-node-id]")).find((candidate) => duplicateDomIdLinks(candidate).length >= 2);
          if (!link)
            return false;
          const links = duplicateDomIdLinks(link);
          if (!links.length || el.contains(links[0]))
            return false;
          ...
          if (normalize(readableText(el)) === name)
            return true;
          const namedRegion = Array.from(el.querySelectorAll("section,[role='region']")).find((candidate) => !isHidden(candidate) && normalize(accessibleName(candidate, "region")) === name);
          if (!namedRegion)
            return false;
          const href = normalize(link.getAttribute("href"));
          const descendantLinks = Array.from(el.querySelectorAll("a[href]"));
          return Boolean(href && descendantLinks.length >= 2 && descendantLinks.every((candidate) => normalize(candidate.getAttribute("href")) === href));
        }
        ...
        function hasAxNodeForElementRole(el, role) {
          return Boolean(axNodeForElementRole(el, role));
        }
        ...
        function isAxConfirmedSingleButtonMenuItemGroup(el) {
          if (implicitRole(el) !== "menuitem")
            return false;
          const node = axNodeForElementRole(el, "menuitem");
          if (!node || node.childIds?.length !== 1)
            return false;
          const child = accessibilityNodeById.get(normalize(node.childIds[0]) || "");
          return Boolean(child && !child.ignored && ["button", "link"].includes(normalizedAxRole(child.role) || "") && child.properties?.focusable === true);
        }
        ...
        function isExpandedTabWithControlledPanel(el, role = implicitRole(el)) {
          if (role !== "tab")
            return false;
          const tablist = el.closest?.("[role='tablist']");
          if (!tablist || isHidden(tablist))
          ...
          if (accessibilityNodes.length) {
            ...
            if (!axNode || normalizedAxRole(axNode.role) !== "tab")
              return false;
        ...
        function headingButtonSuppressesGroupContext(heading, button) {
          if (!heading || implicitRole(heading) !== "heading")
            return false;
          if (!button || implicitRole(button) !== "button")
            return false;
          ...
          if (listItem && isListItem(listItem) && listItem.contains(heading))
            return false;
          const headingTag = heading.tagName?.toLowerCase() || "h2";
          const headingLevel = Number.parseInt(heading.getAttribute("aria-level") || headingTag.slice(1), 10) || 2;
          const ownerRegion = heading.parentElement;
          const hasNamedOwnerRegion = ownerRegion && implicitRole(ownerRegion) === "region" && normalize(accessibleName(ownerRegion, "region") || readableText(ownerRegion));
          if (!hasNamedOwnerRegion && headingLevel < 4) {
            return false;
          }
          const controlled = resolveIdRef(button.getAttribute("aria-controls"));
          if (!controlled || !["region", "group"].includes(implicitRole(controlled)))
            return false;
          if (!isHidden(controlled))
            return false;
          if (controlled.contains?.(button) || heading.contains?.(controlled))
            return false;
        ...
        function axConfirmedLabelledEmbeddedFrameAnnouncement(el) {
          if (el?.tagName?.toLowerCase() !== "iframe")
            return void 0;
          ...
          if (!node || normalize(node.name) !== groupName)
            return void 0;
        ...
        function axConfirmedListItemEmbeddedFrameAnnouncement(el) {
          if (el?.tagName?.toLowerCase() !== "iframe")
            return void 0;
          ...
          if (!title)
            return void 0;
          const node = axNodeForElementRole(el, "frame");
          if (!node || normalize(node.name) !== title)
            return void 0;
          const listItem = el.closest("li,[role='listitem']");
          if (!listItem || listItem.closest("li,[role='listitem']") !== listItem)
            return void 0;
          const buttons = Array.from(listItem.querySelectorAll("button,[role='button']")).filter((button2) => !isHidden(button2));
          if (buttons.length !== 1)
            return void 0;
          const button = buttons[0];
          const buttonName = normalize(accessibleName(button, "button") || readableText(button));
          if (!buttonName || !/\bvideo\b/iu.test(buttonName))
            return void 0;
          if (!(el.compareDocumentPosition(button) & el.ownerDocument.defaultView.Node.DOCUMENT_POSITION_FOLLOWING)) {
            return void 0;
          }
          return el.hasAttribute("srcdoc") ? "frame 0" : `${title} - Youtube, frame`;
        }
        function listItemEmbeddedFrameForControl(el) {
          const listItem = el?.closest?.("li,[role='listitem']");
          if (!listItem)
            return void 0;
          return Array.from(listItem.querySelectorAll("iframe")).find((iframe) => axConfirmedListItemEmbeddedFrameAnnouncement(iframe));
        }
        ...
        function accessibleName(el, role) {
          ...
          if (role === "link") {
            ...
            if (duplicateDomIdName)
              return duplicateDomIdName;
        ...
        function implicitRole(el) {
          ...
          if (explicit === "region" && axPoliteRegionText(el, "region")) {
            return "text";
          }
          if (explicit === "combobox" && isAxConfirmedComboboxWrapperYieldingToTextbox(el)) {
            return "";
          }
          ...
          if (isScanRootLeadingFocusableIframeStop(el))
            return "group";
          if (axConfirmedLabelledEmbeddedFrameAnnouncement(el))
            return "group";
          if (axConfirmedListItemEmbeddedFrameAnnouncement(el))
            return "group";
          ...
          if (tag === "label" && isAxConfirmedStandaloneLabelTextGroup(el))
            return "group";
          ...
          if (isAxConfirmedEmptyNamedGenericGroup(el))
            return "group";
        ...
        function hasNonEmptyMediaSurface(el) {
          const tag = el?.tagName?.toLowerCase();
          if (tag === "img") {
            return Boolean(normalize(el.getAttribute("src")) || hasAxRole(el, "image"));
          }
          if (tag === "svg") {
            const hasGraphicChild = Array.from(el.children || []).some((child) => child.nodeType === Node.ELEMENT_NODE && !isHidden(child));
            return hasGraphicChild || hasAxRole(el, "image");
          }
          return hasAxRole(el, "image");
        }
        function isFocusableUnsupportedFooterMediaAncestor(el, boundary) {
        ...
        function isUnlabeledFooterMediaImageElement(el, role = implicitRole(el), boundary) {
          if (role !== "image")
            return false;
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
          if (el.closest?.("a[href], button, [role='link'], [role='button']"))
            return false;
          if (accessibleName(el, "image"))
            return false;
        ...
        function visibleElementsInTreeOrder(root) {
          if (!root || root.nodeType !== Node.ELEMENT_NODE)
            return [];
          const elements = [];
          const walker = document.createTreeWalker(root, root.ownerDocument.defaultView.NodeFilter.SHOW_ELEMENT);
          let node;
          while (node = walker.nextNode()) {
            if (!isHidden(node))
              elements.push(node);
        ...
        function hasPriorStaticFooterContent(footer, before) {
          for (const candidate of visibleElementsInTreeOrder(footer)) {
            if (candidate === before || before.contains?.(candidate))
              break;
            if (candidate.contains?.(before))
              continue;
            if (candidate.closest?.("a[href], button, [role='link'], [role='button']"))
              continue;
            if (candidate.querySelector?.("a[href], button, [role='link'], [role='button']"))
              continue;
            const role = implicitRole(candidate);
            if (!["heading", "paragraph", "text"].includes(role))
              continue;
            if (normalize(readableText(candidate) || candidate.textContent))
              return true;
          }
          return false;
        }
        function nextVisibleSiblingAcrossFooterAncestors(el, footer) {
        ...
        function isTextualFooterLinkOrButton(control) {
          const role = implicitRole(control);
          if (role !== "link" && role !== "button")
            return false;
          if (control.querySelector?.("img, svg, [role='img'], picture, canvas"))
            return false;
          return Boolean(normalize(accessibleName(control, role) || readableText(control)));
        }
        function hasFooterTextControlBank(el) {
          const controls = Array.from(el?.querySelectorAll?.("a[href], button, [role='link'], [role='button']") || []).filter((control) => !isHidden(control) && isTextualFooterLinkOrButton(control));
          if (controls.length < 2)
            return false;
          const firstSubstantive = visibleElementsInTreeOrder(el).find((candidate) => {
            if (candidate === el)
              return false;
            if (normalize(readableText(candidate) || candidate.textContent))
              return true;
            if (candidate.querySelector?.("img, svg, [role='img'], picture, canvas"))
              return true;
        ...
        function isFooterUnnamedMediaBeforeTextControlBankImage(el, role) {
          if (!isUnlabeledFooterMediaImageElement(el, role))
            return false;
          const footer = el.closest?.("footer");
          if (!footer || footer.tagName?.toLowerCase() !== "footer")
            return false;
          for (let bank = el.parentElement; bank && bank !== footer; bank = bank.parentElement) {
            if (isFocusableUnsupportedFooterMediaAncestor(bank, footer))
              return false;
            const media = Array.from(bank.querySelectorAll?.("svg, img, [role='img']") || []).filter((candidate) => isUnlabeledFooterMediaImageElement(candidate, implicitRole(candidate), footer));
            if (media.length < 2 || !media.includes(el))
              continue;
            const following = nextVisibleSiblingAcrossFooterAncestors(bank, footer);
            if (!following || !hasFooterTextControlBank(following))
              continue;
        ...
        function isAxConfirmedCollapsedCarouselListbox(el) {
          if (el?.getAttribute?.("role") !== "listbox" || isHidden(el))
            return false;
          if (!hasAxRole(el, "listbox"))
            return false;
          const list = el.closest("ul,ol,[role='list']");
          if (!list || implicitRole(list) !== "list")
            return false;
          const listboxes = Array.from(list.querySelectorAll("[role='listbox']")).filter((candidate) => !isHidden(candidate));
          if (listboxes.length !== 1 || listboxes[0] !== el)
            return false;
          const visibleOptions = Array.from(el.querySelectorAll("[role='option']")).filter((option) => !isHidden(option));
          if (!visibleOptions.length)
            return false;
          const owner = list.closest("section,nav,[role='region'],[role='navigation']") || list.parentElement;
          const controlNames = Array.from(owner?.querySelectorAll("button,[role='button']") || []).filter((button) => !isHidden(button)).map((button) => normalize(accessibleName(button, "button") || readableText(button)));
          return controlNames.some((name) => /^previous\b/iu.test(name || "")) && controlNames.some((name) => /^next\b/iu.test(name || ""));
        }
        function hasAxConfirmedCollapsedCarouselListbox(list) {
          if (implicitRole(list) !== "list")
            return false;
          return Array.from(list.querySelectorAll("[role='listbox']")).some((candidate) => isAxConfirmedCollapsedCarouselListbox(candidate));
        }
        ...
        function positionInSet(el, role) {
          ...
          if (role === "menuitem") {
            const items = Array.from(el.closest("[role='menubar']")?.querySelectorAll("[role='menuitem']") || []).filter((item) => !isHidden(item));
          ...
          if (role === "text" && isFirstSplitTextListItemBlock(el) && !hasAxExposedAnonymousSvgListItemContent(el.closest("li,[role='listitem']"), "listitem") && !el.closest("li,[role='listitem']")?.querySelector("img, [role='img'], svg[aria-label]")) {
            const { listItem, siblings } = semanticListContext(el);
            const index = siblings.indexOf(listItem);
            return index >= 0 ? index + 1 : void 0;
          }
        ...
        function setSize(el, role) {
          ...
          if (role === "list") {
            ...
            return flattenedSize ?? (hasAxConfirmedCollapsedCarouselListbox(el) ? 1 : listSummaryChildren(el).length || void 0);
          }
          if (role === "group" && axConfirmedListItemEmbeddedFrameAnnouncement(el)) {
            return semanticListContext(el).siblings.length || void 0;
          }
          ...
          if (role === "menuitem") {
            return Array.from(el.closest("[role='menubar']")?.querySelectorAll("[role='menuitem']") || []).filter((item) => !isHidden(item)).length || void 0;
          }
          if (role === "radio")
        ...
        function axRawChildNodes(node) {
          return (node?.childIds || []).map((childId) => accessibilityNodeById.get(normalize(childId) || "")).filter((child) => Boolean(child));
        }
        ...
        function axNativeInlineChildBoundaryListItemAnnouncements(el) {
          ...
          if (!directLinks.length)
            return void 0;
          const metadataElements = Array.from(el.children || []).filter((child) => Boolean(generatedParentheticalMetadataText(child)));
          const noMetadataBoundaryShape = isNoMetadataNativeInlineListItemBoundaryShape(el, directLinks, metadataElements);
          if (!metadataElements.length && !noMetadataBoundaryShape)
            return void 0;
          ...
          for (const child of Array.from(el.childNodes || [])) {
            ...
            if (tag === "br")
              continue;
            if (directLinks.includes(child) || metadataElements.includes(child))
              continue;
          ...
          if (!hasEmphasisBoundary)
            return void 0;
          const axListItem = axNodeForElementRole(el, "listitem");
          if (!axListItem || normalize(axListItem.name))
            return void 0;
          if (axListItem.properties?.focusable === true)
            return void 0;
          const axChildren = axRawChildNodes(axListItem);
          if (axChildren.length < 5)
            return void 0;
          const [markerNode, ...contentNodes] = axChildren;
          ...
          function pushText(value) {
            ...
            if (!text || !/[\p{L}\p{N}]/u.test(text))
              return;
            if (metadataText) {
              const separator = needsBoundary(metadataText, text) ? " " : /^[,.;:!?)]/u.test(text) ? "" : " ";
              metadataText = `${metadataText}${separator}${text}`;
            } else {
              fragments.push(text);
            }
          }
          function axStaticText(node) {
            return normalize(node.name);
          }
          function axGeneratedMetadataText(node) {
            const metadataElement = metadataElements[metadataIndex];
            if (!metadataElement)
              return void 0;
            if (normalize(node.domNodeId) && normalize(metadataElement.getAttribute("data-sr-dom-node-id")) && normalize(node.domNodeId) !== normalize(metadataElement.getAttribute("data-sr-dom-node-id"))) {
              return void 0;
            }
            const fragments2 = [];
            function collect(current) {
              if (!current)
                return;
              if (current.ignored) {
                for (const child of axRawChildNodes(current))
                  collect(child);
                return;
              }
              if (normalizedAxRole(current.role) === "statictext") {
                const text2 = normalize(current.name);
                if (text2)
                  fragments2.push(text2);
                return;
              }
              for (const child of axRawChildNodes(current))
                collect(child);
            ...
            collect(node);
            const text = normalize(fragments2.join(""));
            const expected = generatedParentheticalMetadataText(metadataElement);
            return text && expected && text === expected ? text : void 0;
          }
          function pushAxContent(node) {
            ...
            if (role === "strong" || role === "emphasis") {
              ...
              if (textFragments.length > 1 && axRawChildNodes(node).some((child) => normalizedAxRole(child.role) === "linebreak")) {
            ...
            if (role === "generic" || role === "none") {
              const metadata = axGeneratedMetadataText(node);
              if (!metadata) {
                disallowed = true;
                return;
              }
          ...
          if (noMetadataBoundaryShape && (metadataElements.length || directLinks.length !== 1 || noMetadataEmphasisBeforeLinkCount < 2 || !noMetadataTextAfterLinkBeforeBreak || !noMetadataLineBreakSeen || !noMetadataTextAfterBreak)) {
            return void 0;
          }
        ...
        function isNoMetadataNativeInlineListItemBoundaryShape(el, directLinks, metadataElements) {
          if (metadataElements.length || directLinks.length !== 1)
            return false;
          if (el.querySelector("img, svg, [role='img'], [role='image']"))
            return false;
          const tokens = [];
          for (const child of Array.from(el.childNodes || [])) {
            if (child.nodeType === Node.TEXT_NODE) {
          ...
          const linkIndexes = tokens.map((token, index) => token === "link" ? index : -1).filter((index) => index >= 0);
          ...
          if (breakIndex < 0)
            return false;
          const emphasisBeforeLink = tokens.slice(0, linkIndex).filter((token) => token === "emphasis").length;
          if (emphasisBeforeLink < 2)
            return false;
          const hasPlainTextBeforeLink = tokens.slice(0, linkIndex).some((token) => token === "text");
          const hasPostLinkText = tokens.slice(linkIndex + 1, breakIndex).some((token) => token === "text");
        ...
        function axMarkerOnlyListItemStopAnnouncement(el) {
          ...
          if (!nativeAxListMarkerText(markerNode))
            return void 0;
          const linkTrailingTextAnnouncement = axMarkerPrefixedLinkTrailingTextListItemAnnouncement(el);
        ...
        function axMarkerLinkTrailingTextListItemAnnouncements(el) {
          if (!isListItem(el) || el.tagName?.toLowerCase() !== "li")
            return void 0;
          if (el.getAttribute("data-sr-marker-content") !== "normal")
            return void 0;
          ...
          const linkChildren = Array.from(children[0].children || []).filter((child) => !isHidden(child));
          ...
          if (axLinkChildren.length !== 1 || !["strong", "emphasis"].includes(normalizedAxRole(axLinkChildren[0].role) || "")) {
            return void 0;
          }
          if (normalize(axLinkChildren[0].domNodeId) !== normalize(linkChildren[0].getAttribute("data-sr-dom-node-id"))) {
            return void 0;
          }
        ...
        function axMarkerPrefixedLinkTrailingTextListItemAnnouncement(el) {
          if (axMarkerLinkTrailingTextListItemAnnouncements(el)?.length)
            return void 0;
          if (!isListItem(el) || el.tagName?.toLowerCase() !== "li")
            return void 0;
          if (el.getAttribute("data-sr-marker-content") !== "normal")
        ...
        function generatedParentheticalMetadataText(node) {
          if (!node || node.nodeType !== Node.ELEMENT_NODE || isHidden(node))
            return void 0;
        ...
        function directLinkGeneratedMetadataFragments(el) {
          ...
          for (const child of Array.from(el.childNodes || [])) {
            ...
            const metadata = generatedParentheticalMetadataText(child);
            if (metadata) {
              if (!sawLink) {
                disallowed = true;
                break;
        ...
        function productCardPricePairText(el) {
          ...
          if (unit?.tagName?.toLowerCase() !== "p" || isHidden(unit))
            return void 0;
          if (unit.querySelector(interactiveSelector))
            return void 0;
          const unitPrice = normalize(textWithoutInteractive(unit) || readableText(unit));
          if (!price || !unitPrice)
            return void 0;
          if (!/^[£$€]\s*\d+(?:[.,]\d+)?$/u.test(price))
            return void 0;
          if (!/^[£$€]\s*\d+(?:[.,]\d+)?\s*(?:\/\s*|per\s+)(?:each|kg|g|l|ml|item)\b/iu.test(unitPrice)) {
            return void 0;
          }
          const card = el.closest("[role='region'],[role='group'],section,article");
          if (!card || !card.querySelector("button[aria-label],input[type='number']"))
            return void 0;
          const quantityControl = Array.from(card.querySelectorAll("button[aria-label],input[aria-label]")).some((control) => /^(?:add|increase|decrease|remove)\b/iu.test(normalize(control.getAttribute("aria-label")) || ""));
          return quantityControl ? `${price} ${unitPrice}` : void 0;
        }
        function isConsumedProductCardUnitPrice(el) {
          if (el?.tagName?.toLowerCase() !== "p")
            return false;
          const previous = el.previousElementSibling;
        ...
        function captureElement(el) {
          ...
          const axWhitespaceButtonName = axWhitespaceAdjustedButtonName(el, role, name);
          const axNativeButtonSymbolSpacingName = role === "button" ? axConfirmedNativeButtonSymbolSpacingName(el, role, axWhitespaceButtonName || name) : void 0;
          ...
          const suppressPaginationControlPosition = isPaginationNavigationControl(el, role) || role === "button" && Boolean(listItemEmbeddedFrameForControl(el));
          const position = markerSeparatedListLink || largePlainListItem || suppressPaginationControlPosition ? void 0 : positionInSet(el, role);
          const size = markerSeparatedListLink || largePlainListItem || suppressPaginationControlPosition ? void 0 : setSize(el, role);
          const rect = el.getBoundingClientRect();
          ...
          const suppressSingleButtonNestedGenericGroup = isAxConfirmedSingleButtonNestedGenericGroupControl(el, role, name);
          const collapsedVisibleControlledRegionButton = controlsVisibleAxRegion(el, role);
          const retainGroupedCollapsedAriaRoleButtonWithListPosition = isLandmarkListPositionedCollapsedAriaRoleButtonWithVisibleRegion(el, role, position, size);
          const suppressGroupedCollapsedAriaRoleButtonGroup = collapsedAriaRoleButtonInGroupedContext(el, role) && !retainGroupedCollapsedAriaRoleButtonWithListPosition;
          ...
          const value = tag === "select" ? nativeSelectValue(stateEl) : nativeRangeValue ? nativeRangeValue : role === "listbox" && isAxConfirmedCollapsedCarouselListbox(el) ? void 0 : selectedListboxOption ? accessibleName(selectedListboxOption, "option") || readableText(selectedListboxOption) : "value" in stateEl && stateEl.value ? stateEl.value : void 0;
          ...
          const descriptor = {
            role,
            name: carouselControlName || visibleTextEllipsisButtonName(el, role) || axNativeInputButtonName || (role === "listbox" && isAxConfirmedCollapsedCarouselListbox(el) ? void 0 : nativeInputComboboxPlaceholderName ? void 0 : announcementName) || nativeSelectTitleName || focusableFeedbackGroupText,
            ...
            roleDescription: role === "list" && tag === "dl" ? "definition list" : role === "radiogroup" ? "radio group" : role === "group" && isLabelledAriaTabGroup(el) ? "tab group" : role === "button" && nativeDetailsSummary ? "disclosure triangle" : role === "contentinfo" && isSimpleNativeFooter(el) ? "footer" : role === "alert" && isEmptyAlertBeforeDialog(el) ? "group" : role === "region" && isHeaderNavigationListControlledEmptyRegion(el) ? "empty region" : role === "paragraph" && el.getAttribute("tabindex") === "-1" && hasStructuredListItemContent(el.closest("li,[role='listitem']")) ? "empty group" : role === "group" && isNamedEmptyDecorativeMediaGroup(el, role) ? "empty group" : role === "group" && isAxConfirmedEmptyNamedGenericGroup(el) ? "empty group" : role === "group" && isAxConfirmedExplicitEmptyNamedRoleGroup(el) ? "empty group" : role === "group" && isScanRootLeadingFocusableIframeStop(el) ? "empty group" : ariaRoleDescriptionForDescriptor(el, role),
            ...
            parenthesizedCollectionPosition: role === "menuitem" || role === "term" && (isWrappedDefinitionListItem(el) || isSimpleDirectDefinitionListItem(el) || isDirectListBackedDefinitionItem(el) || Boolean(definitionListDisclosureButton(el))) || role === "group" && (isFocusableStructuredListItemGroup(el) || isFocusableImageListItem(el)) || void 0,
            ...
            suppressAutocomplete: isNativeSearchFormLabelStopInput(el, role) || isTextboxYieldedFromAxComboboxWrapper(el, role) || void 0,
            ...
            standaloneLabelTextGroupText: role === "group" && isAxConfirmedStandaloneLabelTextGroup(el) ? normalize(textWithoutInteractive(el) || readableText(el)) : void 0,
            ...
            labelledEmbeddedFrameAnnouncement: role === "group" ? axConfirmedLabelledEmbeddedFrameAnnouncement(el) : void 0,
            listItemEmbeddedFrameAnnouncement: role === "group" ? axConfirmedListItemEmbeddedFrameAnnouncement(el) : void 0,
            tabExpandedState: role === "tab" && (isPreviewFrameTab(el, role) || isCodePanelTab(el, role) || isExpandedTabWithControlledPanel(el, role)) ? true : void 0,
            ...
            axMarkerLinkTrailingTextListItemAnnouncements: role === "listitem" ? axMarkerLinkTrailingTextListItemAnnouncements(el) : void 0,
            axMarkerPrefixedLinkTrailingTextListItemAnnouncement: role === "listitem" ? axMarkerPrefixedLinkTrailingTextListItemAnnouncement(el) : void 0,
            ...
            suppressContextEnd: role === "banner" && isEmptyContextStop(el, role) || role === "banner" && isBannerWithTerminalFocusableOffscreenPreMainWrapperGroup(el, role) || role === "region" && isEmptyNamedRegionStop(el, role) || role === "tooltip" || role === "group" && isNamedEmptyDecorativeMediaGroup(el, role) || role === "group" && isAxConfirmedEmptyNamedGenericGroup(el) || role === "group" && isAxConfirmedExplicitEmptyNamedRoleGroup(el) || role === "group" && isScanRootLeadingFocusableIframeStop(el) || role === "group" && isAxConfirmedFocusableOffscreenPreMainWrapperGroup(el) || role === "group" && Boolean(compactInputActionGroupLabel(el)) || shouldSuppressSingletonDocumentArticleEnd(el, role) || role === "group" && isButtonShellClusterGroup(el) || role === "group" && isButtonShellGroup(el) || role === "group" && isFocusableImageListItem(el) || role === "group" && isFocusableStructuredListItemGroup(el) || role === "group" && isAxConfirmedFocusableFeedbackGroup(el) || role === "group" && isFocusableRichTextParagraphGroup(el) || role === "group" && isFocusableHeadingRichTextNavigationGroup(el) || role === "group" && isFocusableGenericListItemDescendantGroup(el) || role === "group" && isFocusableSummaryPanelGroup(el, role) || role === "group" && isDecorativeRoleGroupBeforeNativeLinks(el) || role === "group" && isDecorativeGenericGroupBeforeNativeLinks(el) || role === "group" && Boolean(fieldsetPromptText(el)) || role === "term" && (isSimpleDirectDefinitionListItem(el) || isDirectListBackedDefinitionItem(el)) || shouldSuppressNativeMarkerNestedSingletonListEnd(el, role) || role === "dialog" && el.getAttribute("aria-modal") === "true" && (hasExplicitDialogName(el) && modalDialogSummaryItemCount(el) || !hasExplicitDialogName(el) && !readableText(el)) || role === "group" && isCustomElement(el) && hasShadowRootContent(el) && !accessibleName(el, role) ? true : void 0,
            ...table,
          ...
          if (role === "paragraph") {
            ...
            descriptor.name = productPrice || (adjacentValue && paragraphName ? `${paragraphName}${adjacentValue}` : renderedCaseName(el, role, paragraphName) || paragraphName);
            descriptor.text = descriptor.name;
          }
        ...
        function isPaginationNavigationControl(el, role = implicitRole(el)) {
          ...
          if (!navigationName || !/\bpagination\b/i.test(navigationName))
            return false;
          const controlName = normalize(accessibleName(el, role));
          if (!controlName)
            return false;
          if (accessibilityNodes.length) {
            const axNode = axNodeForElementRole(el, role);
        ...
        function isAxExposedAnonymousSvgImage(el, role = implicitRole(el)) {
          if (role !== "image")
            return false;
          if (el?.tagName?.toLowerCase() !== "svg")
            return false;
          if (el.closest?.("[aria-hidden='true'], button, [role='button'], a[href], [role='link']")) {
            return false;
          }
          if (accessibleName(el, role))
            return false;
        ...
        function hasAxExposedAnonymousSvgListItemContent(el, role = implicitRole(el)) {
          if (role !== "listitem")
            return false;
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
          if (hasVisibleInteractiveDescendant(el))
            return false;
          if (!normalize(readableText(el)))
            return false;
          const list = el.parentElement;
          if (!list || implicitRole(list) !== "list")
            return false;
          if (!positionInSet(el, "listitem") || !setSize(el, "listitem"))
            return false;
          const svgs = Array.from(el.querySelectorAll("svg")).filter((svg) => isAxExposedAnonymousSvgImage(svg, "image"));
          if (svgs.length !== 1)
            return false;
          const textLeaves = Array.from(el.querySelectorAll("span, div")).filter((candidate) => !isHidden(candidate) && !candidate.querySelector("svg, img, button, a[href], input, select, textarea") && Boolean(normalize(directOwnText(candidate))));
          return textLeaves.length === 1;
        }
        function isAxExposedAnonymousSvgListItemImage(el, role = implicitRole(el)) {
          if (!isAxExposedAnonymousSvgImage(el, role))
            return false;
          const listItem = el.closest?.("li,[role='listitem']");
          return Boolean(listItem && hasAxExposedAnonymousSvgListItemContent(listItem, "listitem"));
        }
        function isAxExposedAnonymousSvgFormIcon(el, role = implicitRole(el)) {
          if (!isAxExposedAnonymousSvgImage(el, role))
            return false;
          const container = el.closest?.("[aria-label], label, div, span");
          if (!container || !container.parentElement)
            return false;
          const field = el.parentElement?.closest?.("form, fieldset, [role='group'], [aria-label]");
          if (!field)
            return false;
          const controls = Array.from(field.querySelectorAll("input:not([type='hidden']), select, textarea")).filter((control2) => !isHidden(control2));
          if (controls.length !== 1)
            return false;
          const control = controls[0];
          if (control.closest?.("a[href], button, [role='button'], [role='link']"))
            return false;
          return field.contains(control) && field.contains(el);
        }
        function isAxConfirmedEmptyNamedGenericGroup(el) {
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
          const tag = el.tagName?.toLowerCase();
          if (tag !== "span" && tag !== "div")
            return false;
          if (el.matches(interactiveSelector) || hasVisibleInteractiveDescendant(el))
            return false;
          if (normalize(readableText(el) || el.textContent))
            return false;
          const name = explicitAriaName(el);
          if (!name)
            return false;
          const siblings = Array.from(el.parentElement?.children || []);
          const controls = siblings.filter((sibling) => sibling !== el && sibling.matches?.("input:not([type='hidden']), select, textarea") && !isHidden(sibling));
          if (controls.length !== 1)
            return false;
          const formIcons = siblings.filter((sibling) => sibling !== el).flatMap((sibling) => sibling.tagName?.toLowerCase() === "svg" ? [sibling] : Array.from(sibling.querySelectorAll?.("svg") || [])).filter((svg) => isAxExposedAnonymousSvgImage(svg, "image"));
          if (formIcons.length !== 1)
            return false;
          const axNode = axNodeForElementRole(el, "generic");
          if (!axNode || normalize(axNode.name) !== name)
            return false;
        ...
        function isAxConfirmedExplicitEmptyNamedRoleGroup(el) {
          if (implicitRole(el) !== "group" || el.getAttribute("role") !== "group")
            return false;
          if (normalize(el.getAttribute("aria-roledescription")))
            return false;
          ...
          return Boolean(node && normalize(node.name) === name && (!node.childIds || node.childIds.length === 0));
        }
        function isAxConfirmedFooterSocialListPlaceholderHeading(el, role) {
          ...
          if (!list || implicitRole(list) !== "list")
            return false;
          const items = Array.from(list.children || []).filter((item) => !isHidden(item));
          if (items.length < 2 || items.length > 8)
            return false;
          return items.every((item) => {
            if (implicitRole(item) !== "listitem")
              return false;
            const visibleChildren = Array.from(item.children || []).filter((child) => !isHidden(child));
            if (visibleChildren.length !== 1)
              return false;
            const link = visibleChildren[0];
            return Boolean(implicitRole(link) === "link" && normalize(accessibleName(link, "link")) && link.querySelector("svg,img,picture,[role='img']"));
          });
        }
        function isAxConfirmedSingleButtonNestedGenericGroupControl(el, role = implicitRole(el), name) {
          if (role !== "button")
            return false;
          if (el?.tagName?.toLowerCase() !== "button")
            return false;
          if (el.hasAttribute("aria-expanded") || el.hasAttribute("aria-pressed") || normalizedPopup(el)) {
            return false;
          }
          const parent = el.parentElement;
          if (!parent || implicitRole(parent) !== "group")
            return false;
          const parentName = normalize(accessibleName(parent, "group"));
          if (!parentName)
            return false;
          const visibleChildren = Array.from(parent.children || []).filter((child) => !isHidden(child));
          if (visibleChildren.length !== 1 || visibleChildren[0] !== el)
            return false;
          const parentNode = axNodeForElementRole(parent, "generic");
          const buttonNode = axNodeForElementRole(el, "button");
          if (!parentNode || !buttonNode)
            return false;
          if (normalize(parentNode.name) !== parentName)
            return false;
          if (normalize(buttonNode.name) !== normalize(name || accessibleName(el, role)))
            return false;
          if (buttonNode.properties?.focusable !== true)
            return false;
          const childIds = parentNode.childIds || [];
          return childIds.length === 1 && normalize(childIds[0]) === normalize(buttonNode.nodeId);
        }
        function isAxConfirmedComboboxWrapperYieldingToTextbox(el) {
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
          ...
          if (el.matches("input, select, textarea"))
            return false;
          const inputs = Array.from(el.querySelectorAll("input")).filter((input2) => {
            if (isHidden(input2))
              return false;
            const type = (input2.getAttribute("type") || "text").toLowerCase();
            return type === "text" || type === "search";
          });
          if (inputs.length !== 1)
            return false;
          const input = inputs[0];
          if (!normalize(input.getAttribute("placeholder")) && !normalize(input.value))
            return false;
          const wrapperNode = axNodeForElementRole(el, "combobox");
          const textboxNode = axNodeForElementRole(input, "textbox");
          if (!wrapperNode || !textboxNode)
            return false;
          if (wrapperNode.properties?.focusable === true)
            return false;
          if (textboxNode.properties?.focusable !== true)
            return false;
          const descendants = axDescendants(wrapperNode);
          return descendants.some((node) => normalize(node.nodeId) === normalize(textboxNode.nodeId));
        }
        function isTextboxYieldedFromAxComboboxWrapper(el, role = implicitRole(el)) {
          if (role !== "textbox")
            return false;
          const wrapper = el?.parentElement;
          return Boolean(wrapper && isAxConfirmedComboboxWrapperYieldingToTextbox(wrapper));
        }
        function isAxConfirmedStandaloneLabelTextGroup(el) {
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
          if (el.tagName?.toLowerCase() !== "label")
            return false;
          if (hasVisibleInteractiveDescendant(el))
            return false;
          const labelName = explicitAriaName(el);
          const visibleText = normalize(textWithoutInteractive(el) || readableText(el));
          if (!labelName || !visibleText)
            return false;
          const controlId = normalize(el.getAttribute("for"));
          if (controlId)
            return false;
          const axNode = axNodeForElementRole(el, "labeltext");
          if (!axNode || normalize(axNode.name) !== labelName)
            return false;
          return axDescendants(axNode).some((node) => !node.ignored && normalizedAxRole(node.role) === "statictext" && normalize(node.name) === visibleText);
        }
        function axWhitespaceAdjustedButtonName(el, role, name) {
          if (role !== "button" || !name)
            return void 0;
          if (el?.tagName?.toLowerCase() !== "button")
            return void 0;
          ...
          const compact = (value) => normalize(value)?.replace(/\s+/g, "");
          if (compact(axName) !== compact(name))
            return void 0;
          const visibleChildren = Array.from(el.children || []).filter((child) => !isHidden(child));
          if (visibleChildren.length < 2)
            return void 0;
          if (visibleChildren.some((child) => {
            const tag = child.tagName?.toLowerCase();
            return !["span", "strong", "b", "em", "i"].includes(tag) || child.children?.length;
          })) {
            return void 0;
          }
          return axName;
        }
        function axPoliteRegionText(el, role = implicitRole(el)) {
          if (role !== "region")
            return void 0;
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return void 0;
          ...
          if (normalize(accessibleName(el, role)))
            return void 0;
          if (normalize(el.getAttribute("aria-live")) !== "polite")
            return void 0;
          if (hasVisibleInteractiveDescendant(el))
            return void 0;
          const text = normalize(readableText(el) || el.textContent);
          ...
          const staticText = descendants.filter((node) => normalizedAxRole(node.role) === "statictext").map((node) => normalize(node.name)).filter((fragment) => Boolean(fragment));
          return normalize(staticText.join(" ")) === text ? text : void 0;
        }
        function isUnnamedAxPoliteLiveRegion(el, role = implicitRole(el)) {
          if (role !== "region")
            return false;
          if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
            return false;
          ...
          if (normalize(el.getAttribute("aria-live")) !== "polite")
            return false;
          const axNode = axNodeForElementRole(el, "generic");
          return Boolean(axNode && !normalize(axNode.name) && axNode.properties?.live === "polite");
        }
        ...
        function isStopElement(el) {
          ...
          if (role === "listitem" && hasAxExposedAnonymousSvgListItemContent(el, role)) {
            return false;
          }
          ...
          if (isUnnamedAxPoliteLiveRegion(el, role)) {
            return false;
          }
          ...
          if (role === "image" && !accessibleName(el, role) && !isInformativeUnlabeledCmsImage(el) && !isAxConfirmedRegionIntroImage(el) && !isAxConfirmedHeadingIntroImage(el) && !isAxConfirmedCardIntroImage(el) && !isAxExposedAnonymousSvgListItemImage(el, role) && !isAxExposedAnonymousSvgFormIcon(el, role) && !isLeadingUnnamedTabPanelImage(el, role) && !isFooterUnnamedMediaBeforeTextControlBankImage(el, role) && !hasStructuredListItemContent(el.closest("li,[role='listitem']"))) {
            return false;
          }
        ...
        function shouldDescendIntoStop(el) {
          ...
          if (role === "listitem") {
            ...
            if (axNativeMarkerListItemFragmentAnnouncements(el))
              return false;
            if (axMarkerLinkTrailingTextListItemAnnouncements(el))
              return false;
            if (namedNavigationListItemGroupedLinkAnnouncements(el))
              return false;
            ...
            if (axSplitHeaderListItemAnnouncement(el))
              return true;
            if (hasAxExposedAnonymousSvgListItemContent(el, role))
              return true;
        ...
        function collapsedPopupController(container) {
          ...
          if (isVisibleAxControlledRegion(container))
            return null;
          if (isHeaderNavigationListControlledEmptyRegion(container))
            return null;
        ...
        function isCollapsedButtonControllerForNamedAxRegionByReference(controller, region) {
          if (!controller || !region?.id)
            return false;
          if (isHidden(controller) || isHidden(region))
            return false;
          ...
          if (!buttonNode || buttonNode.properties?.expanded !== false)
            return false;
          const regionNode = axNodeForElementRole(region, "region");
          if (!regionNode || !normalize(regionNode.name))
            return false;
        ...
        function visibleAxControlledRegionsForButton(el, role) {
          if (role !== "button")
            return [];
          if (parseBooleanAttribute(el, "aria-expanded") !== false)
            return [];
          ...
          if (!controls)
            return [];
          return controls.split(/\s+/).map((id) => resolveIdRef(id)).filter(Boolean).filter((region) => isVisibleAxControlledRegion(region) && isCollapsedButtonControllerForVisibleAxRegion(el, region));
        }
        function namedAxControlledRegionsForButtonByReference(el, role) {
          if (role !== "button")
            return [];
          if (parseBooleanAttribute(el, "aria-expanded") !== false)
            return [];
          ...
          if (!controls)
            return [];
          return controls.split(/\s+/).flatMap((id) => Array.from(document.querySelectorAll(`[id="${cssEscape(id)}"]`))).filter((region) => isCollapsedButtonControllerForNamedAxRegionByReference(el, region));
        }
        function controlsVisibleAxRegion(el, role) {
          return visibleAxControlledRegionsForButton(el, role).length > 0;
        }
        function isHeaderNavigationListContext(el) {
          const banner = el.closest?.("header, [role='banner']");
          if (!banner || isHidden(banner))
            return false;
          const navigation = el.closest?.("nav, [role='navigation']");
          return Boolean(navigation && !isHidden(navigation) && banner.contains(navigation));
        }
        function isEarlierDuplicateHeaderNavigationBeforeSearch(el) {
          if (implicitRole(el) !== "navigation")
            return false;
          const header = el.closest("header,[role='banner']");
          if (!header || isHidden(header))
            return false;
          const name = normalize(accessibleName(el, "navigation"));
          const contents = normalize(readableText(el));
          if (!name || !contents || !hasAxRole(el, "navigation"))
            return false;
          const laterNavigation = Array.from(header.querySelectorAll("nav,[role='navigation']")).find((candidate) => {
            if (candidate === el || isHidden(candidate))
              return false;
            if (candidate.closest("header,[role='banner']") !== header)
              return false;
            if (!hasAxRole(candidate, "navigation"))
              return false;
            if (normalize(accessibleName(candidate, "navigation")) !== name)
              return false;
            if (normalize(readableText(candidate)) !== contents)
              return false;
            return Boolean(el.compareDocumentPosition(candidate) & el.ownerDocument.defaultView.Node.DOCUMENT_POSITION_FOLLOWING);
          });
          if (!laterNavigation)
            return false;
          return Array.from(header.querySelectorAll("input[type='search'],[role='searchbox']")).some((search) => {
            if (isHidden(search) || !hasAxRole(search, "searchbox"))
              return false;
            const afterEarlier = Boolean(el.compareDocumentPosition(search) & el.ownerDocument.defaultView.Node.DOCUMENT_POSITION_FOLLOWING);
            const beforeLater = Boolean(search.compareDocumentPosition(laterNavigation) & search.ownerDocument.defaultView.Node.DOCUMENT_POSITION_FOLLOWING);
            return afterEarlier && beforeLater;
          });
        }
        function adjacentControlledRegions(el, regions) {
          return regions.filter((region) => {
            if (region.parentElement !== el.parentElement)
              return false;
            return el.nextElementSibling === region || el.previousElementSibling === region;
          });
        }
        function isLandmarkListPositionedCollapsedAriaRoleButtonWithVisibleRegion(el, role, position, size) {
          if (role !== "button")
            return false;
          if (el?.getAttribute?.("role") !== "button")
            return false;
          ...
          if (!listItem || !list || isHidden(listItem))
            return false;
          ...
          if (!isHeaderNavigationListContext(el))
            return false;
          return adjacentControlledRegions(el, namedAxControlledRegionsForButtonByReference(el, role)).some((region) => isHeaderNavigationListControlledEmptyRegion(region));
        }
        function isHeaderNavigationListControlledEmptyRegion(el) {
          if (!isEmptyNamedRegionStop(el, "region"))
            return false;
          const labelledBy = normalize(el.getAttribute("aria-labelledby"));
          if (!labelledBy)
            return false;
          const controllers = Array.from(document.querySelectorAll(`[aria-controls="${cssEscape(el.id)}"]`)).filter((controller) => {
            if (controller.parentElement !== el.parentElement)
              return false;
            if (controller.nextElementSibling !== el)
              return false;
          ...
          return controllers.some((controller) => {
            if (!isHeaderNavigationListContext(controller))
              return false;
        ...
        function splitStandaloneLabelTextGroupAnnouncements(descriptor) {
          const labelText = normalize(descriptor.standaloneLabelTextGroupText);
          if (descriptor.role !== "group" || !labelText)
            return void 0;
          return [generateAnnouncement2(descriptor), labelText];
        }
        ...
        function splitLabelledEmbeddedFrameAnnouncements(descriptor) {
        ...
        function splitListItemEmbeddedFrameAnnouncements(descriptor) {
        ...
        function splitAxMarkerLinkTrailingTextListItemAnnouncements(descriptor) {
          if (descriptor.role !== "listitem" || !descriptor.axMarkerLinkTrailingTextListItemAnnouncements?.length) {
            return void 0;
          }
        ...
        function scanSubtree(root) {
          ...
          function descriptorAnnouncementResult(descriptor, el) {
            const candidates = [
              ...
              {
                source: "split-standalone-label-text-group",
                announcements: splitStandaloneLabelTextGroupAnnouncements(descriptor)
              },
              {
          ...
          function walk(el) {
            if (!el || el.nodeType !== Node.ELEMENT_NODE || isHidden(el))
              return;
            if (isDuplicateDomIdAxNamedLinkRemainder(el))
              return;
            ...
            if (isAxConfirmedNativeSearchFormLabel(el))
              return;
            if (isEarlierDuplicateHeaderNavigationBeforeSearch(el))
              return;
            ...
            const politeRegionText = axPoliteRegionText(el, "region");
            if (politeRegionText) {
              const id = `__sr_el_${stopIndex}_${now()}`;
              stopIndex += 1;
              el.setAttribute("data-sr-id", id);
              emitTraversalStop(id, syntheticTextStop("ax-polite-region-text", el, politeRegionText));
              return;
            }
            ...
            if (isStopElement(el)) {
              ...
              if (descriptor) {
                if (descriptor.axMarkerPrefixedLinkTrailingTextListItemAnnouncement && !descriptor.axNativeMarkerListItemFragmentAnnouncements?.length) {
