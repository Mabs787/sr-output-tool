...
- For OCR/glyph disagreements, rendered HTML, AX text/name, snapshots, screenshots, and source/caption diagnostics may correct `refinedAnnouncements`: `Al` -> `AI`, `APl` -> `API`, `OpenAl` -> `OpenAI`, ellipsis/quote drift, and misread external-link glyphs such as `↗`. Keep raw scan evidence unchanged and do not teach the engine to mimic OCR artifacts.
